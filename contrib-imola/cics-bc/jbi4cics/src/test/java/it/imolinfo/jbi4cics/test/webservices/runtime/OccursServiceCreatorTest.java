...
public class OccursServiceCreatorTest extends BaseCommareaTest {
  private static Log log = LogFactory.getLog(OccursServiceCreatorTest.class);


  public OccursServiceCreatorTest(String arg) {
    super(arg);
  }
  
  public void testOccursServiceCreator() {
    try {
      ...
      log.error("errore in test testOccursServiceCreator", e);
      fail(e.getMessage());
    }
  }
  
  public void testOccurs2ServiceCreator() {
    try {
      // genero il mapping descriptor
      ...
      log.error("errore in test testOccurs2ServiceCreator", e);
      fail(e.getMessage());
    }
  }  
  
  public void testOccurs3ServiceCreator() {
    try {
      // genero il mapping descriptor
      InputStream is = new FileInputStream(testRootCommarea + "/" + "OccursCommarea3.txt");
      CommareaLexer lexer = new CommareaLexer(is);
      CommareaParser parser = new CommareaParser(lexer);
      CommareaBeanMappingDescriptor commareaBeanMappingDescriptor = parser.commarea_definition();
      log.info("commareaBeanMappingDescriptor: " + commareaBeanMappingDescriptor);
