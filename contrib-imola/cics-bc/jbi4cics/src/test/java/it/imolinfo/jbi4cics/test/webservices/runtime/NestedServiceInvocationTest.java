...
public class NestedServiceInvocationTest extends BaseCommareaTest {

  private static Log log = LogFactory.getLog(ServiceInvocationTest.class);

  public static final String DeafultHostCodePage="CP037";

  public NestedServiceInvocationTest() {
  }


  public void testNestedCommareaServiceInvocation(){
    try {
      ...
      serviceDescriptor.setOperationName("provaNestedOperation");
      serviceDescriptor.setServiceName("ProvaNested");
      serviceDescriptor.setServiceInterfacePackageName("it.imolinfo.jbi4cics.test.webservices.utils.generators");
      serviceDescriptor.setServiceInterfaceName("ProvaNestedBeanInterface");
      serviceDescriptor.setServiceNameSpace("urn:it.imolinfo.jbi4cics.test.webservices.utils.generators");
      serviceDescriptor.setInputBeanClassName("ProvaNestedBean");
      serviceDescriptor.setOutputBeanClassName("ProvaNestedBean");
      ...
      // invoco il servizio
      Document response = invokeService(service.getName().getLocalPart(), "/xmlmessages/testNestedMessage.xml" );
      printNode(response);
  ...
  public void testNestedCommarea2ServiceInvocation(){
    try {
      // genero il mapping descriptor
      ...
      serviceDescriptor.setOperationName("provaNestedOperation");
      serviceDescriptor.setServiceName("ProvaNested2");
      serviceDescriptor.setServiceInterfacePackageName("it.imolinfo.jbi4cics.test.webservices.utils.generators");
      serviceDescriptor.setServiceInterfaceName("ProvaNested2BeanInterface");
      serviceDescriptor.setServiceNameSpace("urn:it.imolinfo.jbi4cics.test.webservices.utils.generators");
      serviceDescriptor.setInputBeanClassName("ProvaNested2Bean");
      serviceDescriptor.setOutputBeanClassName("ProvaNested2Bean");
      serviceDescriptor.setInputMappingDescriptor(commareaBeanMappingDescriptor);
      serviceDescriptor.setOutputMappingDescriptor(commareaBeanMappingDescriptor);
      serviceDescriptor.setCodePage(DeafultHostCodePage);
      ...
      // invoco il servizio
      Document response = invokeService(service.getName().getLocalPart(), "/xmlmessages/testNested2Message.xml" );
      printNode(response);
  ...
  public void testNestedCommarea2ServiceInvocationWithNulls(){
    try {
      // genero il mapping descriptor
      InputStream is = new FileInputStream(testRootCommarea+"/"+"NestedCommarea2.txt");
      CommareaLexer lexer = new CommareaLexer(is);
      ...
      // invoco il servizio
      Document response = invokeService(service.getName().getLocalPart(), "/xmlmessages/testNestedWithNullsMessage.xml" );
      printNode(response);
