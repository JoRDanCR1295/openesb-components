...
public class SEPluginProjectNode extends AbstractNode {
    ...
    private static final class SEPluginProjectNodeChildren extends Children.Keys implements FileChangeListener {
        ...
        private static final String KEY_CONFIG_FOLDER = "CONFIG_FOLDER_Key"; // NOI18N
        ...
        private FileObject getSourceFolder() {
            try {
                ...
                Logger.getLogger(this.getClass().getName()).log(Level.FINE, ex.getMessage(), ex);
                return null;
            }
        }
...
//                newNodes.add(createNode(srcDirFO));
                try {
                    ...
                    newNodes.add(PackageView.createPackageView(srcGroup[0]));
                } catch (Exception ex) {
                    ex.printStackTrace();
                    FileObject srcDirFO = getSourceFolder();
                    newNodes.add(createNode(srcDirFO));
                }
            } else if ( KEY_CONFIG_FOLDER.equals(key)) {
                newNodes.add(new ConfigFilesNode(this.project));
            }
