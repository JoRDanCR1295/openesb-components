...
public class UpdateProjectImpl implements UpdateImplementation {
    ...
    public void saveUpdate(final EditableProperties props) throws IOException {
        ...
        this.cfg.removeConfigurationFragment("data","http://www.netbeans.org/ns/dummy-project/1",true); //NOI18N
        this.cfg.removeConfigurationFragment("data","http://www.netbeans.org/ns/dummy-project/2",true); //NOI18N
    ...
    public synchronized Element getUpdatedSharedConfigurationData () {
        if (cachedElement == null) {
            Element  oldRoot = this.cfg.getConfigurationFragment("data","http://www.netbeans.org/ns/dummy-project/1",true);    //NOI18N
            if (oldRoot != null) {
                Document doc = oldRoot.getOwnerDocument();
                Element newRoot = doc.createElementNS (SEPluginProjectType.PROJECT_CONFIGURATION_NAMESPACE,"data"); //NOI18N
                ...
                Element root = doc.createElementNS (SEPluginProjectType.PROJECT_CONFIGURATION_NAMESPACE,"root");   //NOI18N
                ...
                root = doc.createElementNS (SEPluginProjectType.PROJECT_CONFIGURATION_NAMESPACE,"root");   //NOI18N
                root.setAttribute ("id","test.src.dir");   //NOI18N
                testRoots.appendChild (root);
                newRoot.appendChild (testRoots);
                cachedElement = updateMinAntVersion (newRoot, doc);
            } else {
                oldRoot = this.cfg.getConfigurationFragment("data","http://www.netbeans.org/ns/dummy-project/2",true);    //NOI18N
                if (oldRoot != null) {
                    Document doc = oldRoot.getOwnerDocument();
                    Element newRoot = doc.createElementNS (SEPluginProjectType.PROJECT_CONFIGURATION_NAMESPACE,"data"); //NOI18N
                    copyDocument (doc, oldRoot, newRoot);
    ...
    public synchronized EditableProperties getUpdatedProjectProperties () {
        ...
        if (cachedProperties.get (SEPluginProjectProperties.JAVADOC_ADDITIONALPARAM)==null) {
            cachedProperties.put (SEPluginProjectProperties.JAVADOC_ADDITIONALPARAM,"");    //NOI18N
        }
        ...
        for (int i=0; i< length; i++) {
            ...
            switch (node.getNodeType()) {
                case Node.ELEMENT_NODE:
                    ...
                    newNode = doc.createElementNS(SEPluginProjectType.PROJECT_CONFIGURATION_NAMESPACE,oldElement.getTagName());
    ...
    private static Element updateMinAntVersion (final Element root, final Document doc) {
        NodeList list = root.getElementsByTagNameNS (SEPluginProjectType.PROJECT_CONFIGURATION_NAMESPACE,MINIMUM_ANT_VERSION_ELEMENT);
        if (list.getLength() == 1) {
            Element me = (Element) list.item(0);
            list = me.getChildNodes();
            if (list.getLength() == 1) {
                me.replaceChild (doc.createTextNode(SEPluginProjectProperties.MINIMUM_ANT_VERSION), list.item(0));
                return root;
            }
        }
