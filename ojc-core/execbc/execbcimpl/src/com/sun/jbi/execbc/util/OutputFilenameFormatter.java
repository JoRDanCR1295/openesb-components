...
import com.sun.jbi.execbc.SequenceRegistry;
...
public class OutputFilenameFormatter {
    ...
    public synchronized static String getNextOutputFileName(String fileNamePattern) throws Exception {
        ...
        if ( type == FileNamePatternType.NAME_INVALID )
            throw new Exception(mMessages.getString("OFF_Invalid_pattern", fileNamePattern));
    ...
    public synchronized static String getNextOutputFileName(String fileNamePattern, String aKey, File seqPersistDir) throws Exception {
        ...
        case NAME_WITH_PERSISTED_SEQ:
            ...
            if ( seq == null || seq.trim().length() == 0 )
                throw new Exception(mMessages.getString("OFF_Error_can_not_resolve_persisted_seq", fileNamePattern));
    ...
    private static String fetchSeq(File seqPersistDir, String epKey, String seqVarName) throws Exception {
        ...
        if ( !seqPersistDir.exists() ) {
            ...
            throw new Exception(mMessages.getString("OFF_Invalid_pattern", seqPersistDir.getAbsolutePath()));
        }
        ...
        if ( l.tryLock(50000, TimeUnit.MILLISECONDS) ) {
                    ...
                    if ( fl != null ) {
                        ...
                        if ( randomSeqFile.length() > 0 ) {
                            ...
                            try {
                                ...
                                throw new Exception(mMessages.getString("OFF_Invalid_sequence_value_from_persistence", new Object[] {seqNum, ex}));
                            }
                        }
                        else {
                            seqNum = "0";
                        ...
                        if ( attempted > 5 ) {
                            throw new Exception(mMessages.getString("OFF_Fail_acquire_seq_file_lock", seqFile.getAbsolutePath()));
                        }
                        attempted++;
                        Thread.sleep(1000);
                    }
            ...
            throw new Exception(mMessages.getString("OFF_Timeout_acquire_exclusive_lock", seqFile.getAbsolutePath()));
        }
        return seqNum;
    }
}
