 ...
 * 
 */
public class FTPBCPerformanceMeasurement implements PerformanceMeasurement {

    public static final String FTP_BC_SOURCE_FILTER = ".*ftpbc.*";
    ...
    public TabularData getPerformanceInstrumentationMeasurement(String endpoint) throws OpenDataException, MBeanException {
        ...
        gr.addPattern(FTP_BC_SOURCE_FILTER, endpoint, getPerfCatFilter());
        return gr.fetchData();
    }
    ...
    private String getPerfCatFilter() {
        ...
        for (int i=0; i < FTPBindingLifeCycle.BC_PERF_CATEGORIES.length; i++) {
            perfCatFilter.append(FTPBindingLifeCycle.BC_PERF_CATEGORIES[i]);
            if (i < FTPBindingLifeCycle.BC_PERF_CATEGORIES.length-1) {
