...
public class StreamUtil {
    private StreamUtil(){}
    public static void writeToStream(byte[] array, OutputStream os) throws IOException {
        int pos = 0;
        int bytes = 65536;
        while (pos < array.length) {
            if (array.length - pos < bytes) {
                bytes = array.length - pos;
            }
            os.write(array, pos, bytes);
            pos += bytes;
        }
    }
     ...
     * @exception Exception  If an error occurs during copying
     */
    public static void copyStream(InputStream source, OutputStream dest, int bufferSize)
    throws IOException {
        int bytesToWrite;
