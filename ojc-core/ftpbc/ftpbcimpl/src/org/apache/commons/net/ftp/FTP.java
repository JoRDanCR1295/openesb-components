...
import org.apache.commons.net.ftp.ssl.channel.ControlChannelProcessor;
import org.apache.commons.net.ftp.ssl.channel.DataChannelProcessor;
import org.apache.commons.net.ftp.ssl.channel.SecureChannel;
...
public class FTP extends TelnetClient {
     ...
     ***/
    public static final int COMPRESSED_TRANSFER_MODE = 12;    // We have to ensure that the protocol communication is in ASCII
    ...
    protected String __encoding = null;
    protected StringBuffer __commandBuffer;
    ...
    String _replyString;
    protected String mFTPSecureMechanism;
    protected InetSocketAddress mRemoteAddress;
    protected boolean mCCCEnabled;

    /**
     * for RFC 2228 and RFC 4217
     */
    public static enum FTPType {

        REGULAR_FTP, FTP_OVER_EXPLICIT_SSL, FTP_OVER_IMPLICIT_SSL
    };
    protected FTPType mType = FTPType.REGULAR_FTP;
    protected String mTStore;
    protected String mTStorePassword;
    protected String mKStore;
    protected String mKStorePassword;
    protected ChannelSelectorDispatcher mSelectorThread;
    protected SocketChannel mSocketChannel;
    protected SSLContext mSSLContext;
    protected SocketChannel mControlChannel;
    protected SocketChannel mDataChannel;
    protected ControlChannelProcessor mControlChannelProcessor;
    protected DataChannelProcessor mDataChannelProcessor;
    protected SecureChannel mSSLControlChannel;
    protected SecureChannel mSSLDataChannel;
    protected ChannelSelectorDispatcher mDispatcher;
    protected String mClientAlias;
    protected String mAliasPass;
    protected String mSecMechanism = FTPCommand.FTP_AUTH_TLS;
    ...
    private void __getReply() throws IOException {
        ...
        if (mControlChannelProcessor != null) {
            if (mControlChannel.isConnected()) {
                mControlChannelProcessor.getInput();
                mDispatcher.getSelector().wakeup();
            }
        }
        ...
        if (line == null) {
            throw new FTPConnectionClosedException(
                    "Connection closed without indication.");        // In case we run into an anomaly we don't want fatal index exceptions
        // to be thrown.
        }
        length = line.length();
        if (length < 3) {
...
//            Logger.getLogger(this.getClass().getName()).log(Level.SEVERE, "======>>>>getReply: COMPELET REPLY READ line=[" + line + "]");
        }
...
//         Logger.getLogger(this.getClass().getName()).log(Level.SEVERE, "======>>>>getReply: command= [" + __commandBuffer.toString() + "] RETURN====[" + _replyCode + "]==[ " + line + " ]====>>>>>>");
        }
    }

    /**
     * Connect to the server with the encoding that will be used for the command
     * channel. This method eventually calls #SocketCleint.connect().
     * 
     * @param host
    ...
    public void connect(String host, int port, String encoding)
            throws IOException {
        // explicitly change control encoding
        if ( encoding != null && encoding.trim().length() > 0 && encoding.equals(_controlEncoding)) {
            _controlEncoding = encoding;
        }
        
        if (mType == FTPType.REGULAR_FTP) {
            super.connect(host, port);
        } else if (mType == FTPType.FTP_OVER_EXPLICIT_SSL) {
            mControlChannel = SocketChannel.open();
            mControlChannel.configureBlocking(true);
            InetSocketAddress remoteAddress = new InetSocketAddress(host, port);
            boolean connected = mControlChannel.connect(remoteAddress);
            _connectAction_();
        } else {
            mRemoteAddress = new InetSocketAddress(host, port);
            _connectAction_();
        }
    }
    ...
    public void connect(String host, int port) throws IOException {
        if (mType == FTPType.REGULAR_FTP) {
            ...
            mControlChannel.configureBlocking(true);
            InetSocketAddress remoteAddress = new InetSocketAddress(host, port);
            mControlChannel.connect(remoteAddress);
            mControlChannel.finishConnect();
            _connectAction_();
        } else {
            mRemoteAddress = new InetSocketAddress(host, port);
            _connectAction_();
    ...
    protected void _connectAction_() throws IOException {
        // just create a command line channel here
        if (mType == FTPType.REGULAR_FTP) {
            ...
            _controlOutput = new BufferedWriter(new OutputStreamWriter(
                    getControlOutputStream(), getControlEncoding()));
            __getReply();
            ...
            // explicit SSL initialization
            // get 200 reply and issue AUTH TLS
            _socket_ = mControlChannel.socket();
            _isConnected_ = true;
            _controlInput = new BufferedReader(new InputStreamReader(
                    mControlChannel.socket().getInputStream(), getControlEncoding()));
            _controlOutput = new BufferedWriter(new OutputStreamWriter(
                    mControlChannel.socket().getOutputStream(), getControlEncoding()));
            //__getReply();
            mControlChannelProcessor = null;
            __getReply();
            // If we received code 120, we have to fetch completion reply.
            if (FTPReply.isPositivePreliminary(_replyCode)) {
                __getReply();
            }
            int retcode = auth(getSecurityMechanism());

            if (retcode == 234) {
                // AUTH supported
                // proceed to handshake...
            } else {
                throw new IOException("AUTH TLS return unexpected return code:" + retcode);
            }
            mControlChannel.configureBlocking(false);
            try {
                mDispatcher = new ChannelSelectorDispatcher();
            } catch (Exception e) {
                throw new IOException(
                        "Exception creating SelectableIODispatcher ...");
            }
            try {
                mSSLContext = SecureChannel.createSSLContext(getKeyStoreLoc(),
                        getKeyStorePassword(), getTrustStoreLoc(),
                        getTrustStorePassword(), getClientAlias(), getAliasPass());
            } catch (Exception e) {
                throw new IOException(
                        "Exception creating SSL context with security store info: e=" + e.getMessage());
            }
            try {
                mControlChannelProcessor = new ControlChannelProcessor();
                mSSLControlChannel = SecureChannel.createChannel(mSSLContext,
                        mControlChannel, mDispatcher, mControlChannelProcessor, mControlChannelProcessor);
                mSSLControlChannel.establishSSLSession(); // could block if the channel is not yet registered yet
            } catch (Exception e) {
                e.printStackTrace();
                throw new IOException(
                        "Exception creating SSL protected channel: ");
            }
            // hook up the piped stream from the channel
            _controlInput = new BufferedReader(new InputStreamReader(
                    /*getControlInputStream()*/mControlChannelProcessor.getInputStream(), getControlEncoding()));
            _controlOutput = new BufferedWriter(new OutputStreamWriter(
                    /*getControlOutputStream()*/mControlChannelProcessor.getOutputStream(), getControlEncoding()));
            retcode = pbsz("0");
            ...
            if (retcode == 200) {
                ...
                if (retcode == 200) {
                    ...
                    throw new IOException("PROT() failed, connection aborted.");
                }
            } else {
                throw new IOException("PBSZ() failed, connection aborted.");
            }
        } else {
            try {
                mSSLContext = SecureChannel.createSSLContext(getKeyStoreLoc(),
                ...
                throw new IOException(
                        "Exception creating SSL context with security store info: ");
            }
            SSLSocketFactory ssf = mSSLContext.getSocketFactory();
            _socket_ = ssf.createSocket();
            _socket_.connect(mRemoteAddress);
            _isConnected_ = true;
            _controlInput = new BufferedReader(new InputStreamReader(
     ...
     * voided so that the memory it used may be reclaimed.
     * <p>
     * 
    ...
    public void disconnect() throws IOException {
        if (mType == FTPType.REGULAR_FTP) {
            ...
            if (mDispatcher != null) {
                try {
                    mDispatcher.requestClose();
                }
                catch (Exception e) {
                    // ignore - don't throw up
                }
            }
            if (mControlChannelProcessor != null) {
                try {
                    mControlChannelProcessor.close();
                }
                catch (Exception e) {
                    // ignore - don't throw up
                }
            }
            if (mDataChannelProcessor != null) {
                try {
                    mDataChannelProcessor.close();
                }
                catch (Exception e) {
                    // ignore - don't throw up
                }
    ...
    public String getReplyString() {
        ...
        listEnum = _replyLines.elements();
        while (listEnum.hasMoreElements()) {
            buffer.append((String) listEnum.nextElement());
    ...
    /***************************************************************************
     * A convenience method to send the FTP AUTH command to the server, receive
     ...
     **************************************************************************/
    public int auth(String mechanism) throws IOException {
        return sendCommand(FTPCommand.AUTH, mechanism);
    }

    public int pbsz(String bufsz) throws IOException {
        return sendCommand(FTPCommand.PBSZ, bufsz);
    }

    public int prot(String protcode) throws IOException {
        return sendCommand(FTPCommand.PROT, protcode);
    }

    public int ccc() throws IOException {
        return sendCommand(FTPCommand.CCC);
    }
    ...
    public void setTrustStoreLoc(String loc) {
        mTStore = loc;
    }

    public String getTrustStorePassword() {
        return mTStorePassword;
    }

    public void setTrustStorePassword(String passwd) {
        mTStorePassword = passwd;
    }
    ...
    public void setClientAlias(String alias) {
        mClientAlias = alias;
    }
    ...
    public void setKeyStoreLoc(String loc) {
        mKStore = loc;
    }

    public String getKeyStorePassword() {
        return mKStorePassword;
    }

    public void setKeyStorePassword(String passwd) {
        mKStorePassword = passwd;
    }

    public void setFTPType(FTPType type) {
        mType = type;
    }

    public void enableSSL(boolean isExplicit) {
        mType = isExplicit ? FTPType.FTP_OVER_EXPLICIT_SSL : FTPType.FTP_OVER_IMPLICIT_SSL;
    }
    ...
    public void setSecurityMechanism(String mech) {
        this.mSecMechanism = mech;
    }

    public String getSecurityMechanism() {
        return this.mSecMechanism;
    }

    public OutputStream getDataOutputStream(Socket socket) throws IOException {
        OutputStream os = null;
        switch (mType) {
            case FTP_OVER_IMPLICIT_SSL:
            // ssl socket behave the same as socket
            case REGULAR_FTP:
                os = socket.getOutputStream();
                break;
            case FTP_OVER_EXPLICIT_SSL:
                os = mDataChannelProcessor.getOutputStream();
                break;
            default:
    ...
    public InputStream getDataInputStream(Socket socket) throws IOException {
        InputStream is = null;
        switch (mType) {
            case FTP_OVER_IMPLICIT_SSL:
            // ssl socket behave the same as socket
            case REGULAR_FTP:
                ...
                break;
            case FTP_OVER_EXPLICIT_SSL:
                is = mDataChannelProcessor.getInputStream();
                mDataChannelProcessor.getInput();
                break;
            default:
                throw new IllegalArgumentException("Invalid FTP type: " + mType);
        }
    ...
    public void closeDataInputStream(Socket socket) {
        switch (mType) {
            case FTP_OVER_IMPLICIT_SSL:
            case REGULAR_FTP:
                try {
    ...
    public OutputStream getControlOutputStream() throws IOException {
        ...
        switch (mType) {
            case FTP_OVER_IMPLICIT_SSL:
                os = _socket_.getOutputStream();
            case REGULAR_FTP:
                os = super.getOutputStream();
                break;
            case FTP_OVER_EXPLICIT_SSL:
                os = mCCCEnabled ? mControlChannel.socket().getOutputStream() : mControlChannelProcessor.getOutputStream();
                break;
            default:
                throw new IllegalArgumentException("Invalid FTP type: " + mType);
        }
    ...
    public InputStream getControlInputStream() throws IOException {
        ...
        switch (mType) {
            case FTP_OVER_IMPLICIT_SSL:
                is = _socket_.getInputStream();
                break;
            case REGULAR_FTP:
                is = super.getInputStream();
                break;
            case FTP_OVER_EXPLICIT_SSL:
                is = mCCCEnabled ? mControlChannel.socket().getInputStream() : mControlChannelProcessor.getInputStream();
                break;
            default:
                throw new IllegalArgumentException("Invalid FTP type: " + mType);
        }
    ...
    public SecureChannel getSSLControlChannel() {
        return mSSLControlChannel;
    }

    public SecureChannel getSSLDataChannel() {
        return mSSLDataChannel;
    }
}
