 ...
 * are <code> FTP.ASCII_FILE_TYPE </code> and <code> FTP.IMAGE_FILE_TYPE </code>
 * (which is the same as <code> FTP.BINARY_FILE_TYPE </code>). Because there
 * are at lest 4 different EBCDIC encodings, we have opted not to provide direct
 ...
 * response to a command. When that occurs, the FTP class method encountering
 * that reply will throw an
...
public class FTPClient extends FTP implements Configurable {
	...
	private boolean __storeFile(int command, String remote, InputStream local)
		...
		output = new BufferedOutputStream(getDataOutputStream(socket),
				getBufferSize());
		...
		output.close();
		// if FTP/SSL - do not close the socket
		// because the bytes are should be flushed by async channel write
		// by another thread
		if (mType == FTPType.REGULAR_FTP
				|| mType == FTPType.FTP_OVER_IMPLICIT_SSL)
			socket.close();
                else {
                    OutputStream os = mDataChannel.socket().getOutputStream();
                    try {
                        os.close();
                    }
                    catch (Exception e) {
	...
	private OutputStream __storeFileStream(int command, String remote)
		...
		output = getDataOutputStream(socket);
		if (__fileType == ASCII_FILE_TYPE) {
	...
	protected Socket _openDataConnection_(int command, String arg)
		...
		if (__dataConnectionMode == ACTIVE_LOCAL_DATA_CONNECTION_MODE) {
			...
			if (mType == FTPType.FTP_OVER_IMPLICIT_SSL) {
				SSLServerSocketFactory sssf = mSSLContext
						.getServerSocketFactory();
				server = sssf.createServerSocket();
			} else {
				ssc = ServerSocketChannel.open();
				server = ssc.socket();
			}

			InetSocketAddress serverAddress = new InetSocketAddress(_socket_
					.getLocalAddress().getHostAddress(), 0);

			server.bind(serverAddress, 1);
			...
			if (mType == FTPType.FTP_OVER_IMPLICIT_SSL) {
				...
				mDataChannel = ssc.accept();
				socket = mDataChannel.socket();
			}
			...
			mDataChannel = SocketChannel.open();
			mDataChannel.configureBlocking(true);
			InetSocketAddress remoteAddress = new InetSocketAddress(
					__passiveHost, __passivePort);
			...
			if (mType == FTPType.FTP_OVER_IMPLICIT_SSL) {
				...
				mDataChannel.connect(remoteAddress);
				socket = mDataChannel.socket();
			}
		...
		if (__remoteVerificationEnabled && !verifyRemote(socket)) {
			...
			host1 = mDataChannel.socket().getInetAddress();
		...
		if (mType == FTPType.REGULAR_FTP
				|| mType == FTPType.FTP_OVER_IMPLICIT_SSL) {
			// just return the socket
		} else if (mType == FTPType.FTP_OVER_EXPLICIT_SSL) {
			// handshake over this socket channel
			if ( mDataChannel.isBlocking() )
				mDataChannel.configureBlocking(false);
			try {
				mDataChannelProcessor = new DataChannelProcessor();
				mSSLDataChannel = SecureChannel.createChannel(mSSLContext,
						mDataChannel, mDispatcher, mDataChannelProcessor,
						mDataChannelProcessor);
                                mSSLDataChannel.establishSSLSession(); // could block if channel not yet registered
			} catch (Exception e) {
				throw new IOException(
						"Exception creating SSL protected data channel: ");
			}
		}
		return socket;
	}
	 ...
	 * <code>ACTIVE_LOCAL_DATA_CONNECTION_MODE</code>. No communication with
	 * the FTP server is conducted, but this causes all future data transfers to
	 ...
	 * call {@link #enterLocalActiveMode  enterLocalActiveMode() } if you wish
	 * to return to the normal data connection mode.
	 * <p>
	 * 
	 * @return True if successfully completed, false if not.
	 * @exception FTPConnectionClosedException
	 ...
	 * been opened after a call to
	 * {@link #enterLocalPassiveMode enterLocalPassiveMode()}. This is because
	 * FTPClient sends a PASV command to the server only just before opening a
	 * data connection, and not when you call
	...
	public boolean retrieveFile(String remote, OutputStream local)
		...
		input = new BufferedInputStream(getDataInputStream(socket),
				getBufferSize());
		if (__fileType == ASCII_FILE_TYPE)
	...
	public InputStream retrieveFileStream(String remote) throws IOException {
		...
		input = getDataInputStream(socket);
		if (__fileType == ASCII_FILE_TYPE) {
			// We buffer ascii transfers because the buffering has to
	 ...
	 *             specify the parser Key in an
	 *             {@link  #FTPClientConfig  FTPClientConfig} object instead.
	 */
	...
	private FTPListParseEngine initiateListParsing(FTPFileEntryParser parser,
		...
		engine.readServerList(getDataInputStream(socket));
		closeDataInputStream(socket);
	...
	public FTPFile[] listFiles(FTPFileListParser parser, String pathname)
		...
		results = parser.parseFileList(getDataInputStream(socket), getControlEncoding());
		closeDataInputStream(socket);
	...
	public FTPFileList createFileList(String pathname, FTPFileEntryParser parser)
		...
		FTPFileList list = list = FTPFileList.create(
				getDataInputStream(socket), parser);
		closeDataInputStream(socket);
	...
	public void doCCC() throws IOException {
		if (mType != FTPType.FTP_OVER_EXPLICIT_SSL)
			throw new IllegalStateException(
					"CCC only applicable during a explicit SSL session");

//		if (mCCCEnabled) {
//                    throw new IllegalStateException("CCC already in place.");
//		}

		int retcode = ccc();

		if (retcode == 200) {
			// OK, torn down the SSL stuff for command channel
			// data channel should stay the same

			mCCCEnabled = true;

			mDispatcher.deregisterChannel(mControlChannel);
