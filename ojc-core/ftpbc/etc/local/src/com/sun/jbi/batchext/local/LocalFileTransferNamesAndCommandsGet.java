...
/*
 * @(#)LocalFileTransferNamesAndCommandsGet.java 
 *
...
public class LocalFileTransferNamesAndCommandsGet extends LocalFileTransferNamesAndCommands {
    //private static final Messages mMessages =
    //        Messages.getMessages(LocalFileTransferNamesAndCommandsGet.class);
    private static final Logger mLogger =
            Messages.getLogger(LocalFileTransferNamesAndCommandsGet.class);
    transient private BatchLocal etd = null;
    private long readPosition;
    public LocalFileTransferNamesAndCommandsGet(BatchLocal etd) {
        this.etd = etd;
        readPosition = 0;
    }
    ...
    public void resolveTargetLocation() throws BatchException {
        if ((null != _TargetDirectoryName) && (null != _TargetFileName)) {
            // the target location has been resolved
            return;
        }
        if (!etd.getConfiguration().getTargetDirectoryNameIsPattern()
        && !etd.getConfiguration().getTargetFileNameIsPattern()) {
            ...
            LocalFileName lfn = etd.getProvider().getFirstFileName(
                    etd.getConfiguration().getTargetDirectoryName(),
                    etd.getConfiguration().getTargetDirectoryNameIsPattern(),
                    etd.getConfiguration().getTargetFileName(),
                    etd.getConfiguration().getTargetFileNameIsPattern());
            if (lfn == null) {
                String msg = "Unable to find a file matching the Target Location configuration settings.";
                //if (mLogger.isLoggable(Level.SEVERE)) {
                if (mLogger.isLoggable(Level.WARNING)) {
                    //mLogger.log(Level.SEVERE, msg); // see qai#68068
                    mLogger.log(Level.WARNING, msg);
                }
                throw new BatchException(msg,
                        new FileNotFoundException(
                        "Directory Name: " + etd.getConfiguration().getTargetDirectoryName()
                        + " File name: " + etd.getConfiguration().getTargetFileName()));
            }
            _TargetDirectoryName = lfn.getDirectoryName();
            _TargetFileName = lfn.getFileName();
    ...
    public void resolvePreTransfer() throws BatchException {
        ...
        if ((null != _PreDirectoryName) && (null != _PreFileName)) {
            ...
            return;
        } else if (LocalFileConfiguration.PRETC_COPY.equalsIgnoreCase(_PreTransferCommand)
    ...
    public void resolvePostTransfer() throws BatchException {
        ...
        || LocalFileConfiguration.POSTTC_NONE.equalsIgnoreCase(_PostTransferCommand)) {
            // make sure that it will fail if they are used somewhere by mistake
        ...
        if ((null != _PostDirectoryName) && (null != _PostFileName)) {
            // the name resolution has been performed
            return;
        } else if (LocalFileConfiguration.POSTTC_RENAME.equalsIgnoreCase(_PostTransferCommand)
