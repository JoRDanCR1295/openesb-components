 ...
 */
public class ScriptseInOutProvider extends AbstractInOutProvider {
    /**
     * 
     */
    public ScriptseInOutProvider() {
    ...
    public void processIn(CRLInOut inOut, ExchangeContext ctx)
        ...
        ScriptseEndpoint input = (ScriptseEndpoint) ctx.getEndpointManager()
                                                       .lookupEndpoint(
                inOut.getEndpoint().getServiceName(), inOut.getEndpoint().getEndpointName(), true
            );
        ...
        if (input.getEntryType() == EntryType.REQUEST_REPLY) {
            ...
            log().warning(
                "Service type: " + String.valueOf(input.getEntryType()) + //srvcType +
    ...
    // A <--- (3. X.out)  <---- B
    // A ----> (4. X.done) ---> B
    private void handleRequest(CRLInOut inOut, ExchangeContext ctx, ScriptseEndpoint input)
        throws JBIException {
...
// INF 11811        input.getStatus().incrementReceivedRequests();
        ...
        try {
            ...
            if (log().isLoggable(Level.FINE)) {
                log().fine("requestReply-input: " + ScriptseUtil.print(content));
            }

            processedContent = ScriptseUtil.transform(content, input, ctx);

            if (log().isLoggable(Level.FINE)) {
                log().fine("requestReply-output: " + ScriptseUtil.print(processedContent));
            }
...
// INF 11811        input.getStatus().incrementSentReplies();
    ...
    // A ---> (8. X.done) ---> B
    private void handleFilterInvoke(CRLInOut inOut, ExchangeContext ctx, ScriptseEndpoint input)
        throws JBIException {
...
// INF 11811        input.getStatus().incrementReceivedRequests();
        ...
        try {
            ...
            if (log().isLoggable(Level.FINE)) {
                log().fine("filterRequestReply-input: " + ScriptseUtil.print(content));
            }

            processedContent = ScriptseUtil.transform(content, input, ctx);

            if (log().isLoggable(Level.FINE)) {
                log().fine("filterRequestReply-output: " + ScriptseUtil.print(processedContent));
            }
        } catch (Exception transformEx) {
            inOut.sendError(transformEx);
// INF 11811            input.getStatus().incrementSentErrors();
        ...
        //        ScriptseUtil.propogateTransaction(inOut, newInOut);
    ...
    public void processStatus(CRLInOut msg, ExchangeContext ctx)
        ...
        if (log().isLoggable(Level.INFO)) {
            ...
            if (ExchangeStatus.DONE.equals(status)) {
                log().info("Received DONE status for exchange: " + msg.getExchangeId());
            } else if (ExchangeStatus.ERROR.equals(status)) {
                log().warning("Received ERROR status for exchange: " + msg.getExchangeId());
            }
        }
    }
} //class ScriptseInOutProvider  ends.
