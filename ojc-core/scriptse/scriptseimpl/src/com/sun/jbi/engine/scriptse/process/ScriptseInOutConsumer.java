 ...
 */
public class ScriptseInOutConsumer extends AbstractInOutConsumer {
    /**
     * 
...
// INF 111811            input.getStatus().incrementSentDones();
            ...
            log().log(Level.SEVERE, "Failed to process fault: " + e.getMessage(), e);
            inOut.sendError(e);
...
// INF 111811                }
    ...
    public void processOut(CRLInOut inOut, ExchangeContext ctx)
        ...
        try {
            ...
            ScriptseEndpoint output = input.getInvoke(); // C

            // propogate transaction regardless of status
            ScriptseUtil.propogateTransaction(inOut, originalInOut);

            // 4. Receive Y.out from C
            NormalizedMessage yOut = inOut.getOutMessage();

            if (yOut == null) {
                inOut.sendError(new JBIException("Cannot process NULL out message!"));
                originalInOut.sendError(new JBIException("Received NULL response!"));
...
// INF 111811            output.getStatus().incrementSentDones();
            ...
            try {
                if (log().isLoggable(Level.FINE)) {
                    log().fine("filterRequestReply-response: " + ScriptseUtil.print(content));
                }
                ...
                if (log().isLoggable(Level.FINE)) {
                    log()
                        ...
                        ScriptseUtil.print(processedContent)
                    );
                }
            } catch (Exception transformEx) {
                log().severe("Transformation failed: " + transformEx.getMessage());
...
// INF 111811            input.getStatus().incrementSentReplies();
            ...
            log().log(Level.SEVERE, "Consuming InOut failed: " + e.getMessage(), e);
            originalInOut.sendError(e);
