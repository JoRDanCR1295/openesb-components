...
import com.sun.jbi.engine.scriptse.ScriptseEndpoint.EntryType;
import com.sun.jbi.engine.scriptse.process.ScriptInfoVO;
import com.sun.jbi.engine.scriptse.scrconfig.FilterOneWay;
import com.sun.jbi.engine.scriptse.scrconfig.Input;
import com.sun.jbi.engine.scriptse.scrconfig.Output;
import com.sun.jbi.engine.scriptse.scrconfig.RequestReplyService;
import com.sun.jbi.engine.scriptse.scrconfig.Scriptconfig;
import javax.xml.bind.JAXBContext;
import javax.xml.bind.Unmarshaller;
/**
 * Implementation of {@link EndpointFactory} for Script SE.
 *
 * @author Prashanth B.R
 */
public class ScriptseEndpointFactory implements EndpointFactory
{
  ...
  private static Logger msLogger = Logger.getLogger (ScriptseEndpointFactory.class.getName ());
   ...
   */
  protected void initEndpoint (ScriptseEndpoint ept, EndpointInfo info, String rootPath)
  throws DeploymentException
  {
    try
    {
      msLogger.log(Level.INFO,"THE ROOT PATH OF THE ENDPOINT IS ::"+ rootPath);
...
//                     JBI_DESC_FILE_NAME);
      ...
      if (!lJarFile.exists ())
      {
        msLogger.log(Level.INFO,"Script SU Additional Schemas jar is not there!!: "+
...
//          handler);
      ...
      ScriptConfigReader lReader = new ScriptConfigReader(info, ept, rootPath);
      lReader.parseScriptMap(ScriptsuDescriptorFile);

      if (msLogger.isLoggable (Level.FINE))
      {
        msLogger.fine ("Endpoint initialization complete: "+ String.valueOf (info));
      }
    }
    catch (Exception ioe)
    {
      throw error ("Unexpected I/O error parsing service unit descriptor: "+
          ioe.getMessage (), ioe);
    }
  }
  ...
  private static Templates getTemplates (String rootPath, String fileName)
  {
    ...
    {
      msLogger.log (Level.SEVERE, "Failed to create a new Transformer for " + fileName, e);
    }
    return ret;
  }
  ...
  private static class DescriptorHandler extends DefaultHandler
  {
    ...
    private boolean mHasInvoke = false, mFoundEntry = false, mFoundInvoke = false;

    public DescriptorHandler (EndpointInfo info,
        ScriptseEndpoint ept,
        String rootPath)
    {
      mInfo = info;
      mEndpoint = ept;
    ...
    {
      if (mFoundEntry && mFoundInvoke) return;
      ...
      else if (mInfo.isProvides ())
      {
        ...
        else if (mHasInvoke && mFoundEntry && qName.equals (OUTPUT_ELEMENT))
        {
                    /*
        ...
        if( qName.equals(INPUT_ELEMENT))
        {
          ...
          msLogger.log(Level.INFO, "THE SCRIPT FILE NAME TO BE USED**** \n"+lScrFileName );
          ...
          {
            msLogger.log(Level.SEVERE, "NO SCRIPT FILE NAME IN SU.. ERRONEOUS Condition Please check\n\n");
          }
    ...
    void init (ScriptseEndpoint endpt, Attributes attr)
    {
  ...
  private static class ScriptConfigReader
  {
    ...
    public void parseScriptMap(File aScrFile)
    {
      ...
      try
      {
        ...
        JAXBContext lJaxbCtx = JAXBContext.newInstance(com.sun.jbi.engine.scriptse.scrconfig.ObjectFactory.class);
...
//            }
        ...
        {
          msLogger.log(Level.INFO,"NONE OF THE VALID MESSAGES TYPES ARE SET...PLEASE CHECK..");
        }
      ...
      {
        ex.printStackTrace();
        msLogger.log(Level.SEVERE,"Failed to Parse the File"+ ex.toString());
      }
       ...
       msLogger.log(Level.INFO,"plinkAttr :: "+ plinkAttr);
        msLogger.log(Level.INFO,"portAttr :: "+ portAttr);
         msLogger.log(Level.INFO,"roleAttr :: "+ roleAttr);
          ...
          msLogger.log(Level.INFO,"convert(mInfo.getServiceName()) :: "+ convert(mInfo.getServiceName()));
        msLogger.log(Level.INFO,"convert(mInfo.getInterfaceName()) :: "+ convert(mInfo.getInterfaceName()));
         msLogger.log(Level.INFO,"mInfo.getEndpointName() :: "+ mInfo.getEndpointName());
...
//    void init(ScriptseEndpoint endpt, Output aOp)
//    {
...
//     ScriptseEndpoint createInvoke (Output aOp)
//    {
//      EndpointInfo info =
//          new EndpointInfo (false,
...
}//class ScriptseEndpointFactory  ends.
