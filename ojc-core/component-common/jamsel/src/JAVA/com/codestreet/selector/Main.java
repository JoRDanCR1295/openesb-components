...

			{
				...
				System.out.println(upper + " evaluations of '" + selector
						+ "' in " + ((end - start) / 1000) + " seconds");
                                ...
                                /**
                                 * Sun Microsystems
                                 * Check 1.0.2 syntax case
                                 * 11/23/2005
                                 */
                                java.util.Map map2 = new java.util.HashMap();
                                map2.put("Weight", new NumericValue(new Integer(10)));
                                map2.put("Height", new NumericValue(new Integer(1)));
                                map2.put("Age", new NumericValue(new Integer(1)));
                                String selector102str = "Weight NOT BETWEEN (5) AND (2.23)";
                                Selector selector102 = Selector.getInstance(selector102str);
                                // Evaluate selector
                                Result result102 = selector102.eval(map2);
                                System.out.println("1.0.2 syntax result = " + result102.toString());

                                
                                /**
                                 * Sun Microsystems
                                 * Check bad syntax case
                                 * 11/23/2005
                                 */
                                String badSelector = "SecurityID = 'SUNW' && Quantity > 100 && Volume <= 50.5";
                                Selector fooSelector = Selector.getInstance(badSelector);
