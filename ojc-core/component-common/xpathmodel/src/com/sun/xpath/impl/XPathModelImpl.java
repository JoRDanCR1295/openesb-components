...
import com.sun.xpath.XPathException;
...
import com.sun.xpath.common.MessageManager;
    ...
    public XPathExpression parseXpathExpression(String expression) throws Exception {
        try {
            ...
            if (expr instanceof Expression) {
                ...
                mLogger.log(Level.SEVERE, "Could not parse expression", expression);
            }
        } catch (Exception jxe) {
            //mLogger.log(Level.SEVERE, "Could not parse expression:" + expression, jxe);
            throw jxe;
        }
        return null;
    }
...
//        QName varQName = QName.valueOf(qNameStr);
    ...
    XPathExpression processExtensionFunction(ExtensionFunction extensionFunction)
        ...
        if (functionName.equals(DO_XSL_TRANSFORM)) {
            validateDoXslTransformArgs(arguments);
        }
        if (functionName.equals(DO_XSD_MARSHAL)) {
            validateDoMarshalArgs(arguments);
        }
        if (functionName.equals(DO_XSD_UN_MARSHAL)) {
            validateDoUnMarshalArgs(arguments);
        }
        if (functionName.equals(GetVariablePropertyFunction.NAME)) {
            validateGetVariablePropertyArgs(arguments);
        }
    ...
    /**
     * Validate arguments passed to doXslTransform function.
     */
    private void validateDoXslTransformArgs(Expression[] args) throws XPathException {
        int len = (args == null) ? 0 : args.length;
        if (len < 2) {
            throw new XPathException(mMsgMgr.getString("XPathModelImpl.doXslMissingArgs"));
        }
        else if (len > 2 && len % 2 != 0) {
            throw new XPathException(mMsgMgr.getString("XPathModelImpl.doXslInvalidParams"));
        }
    }

    /**
     * Validate arguments passed to the getVariableProperty function.
     */
    private void validateGetVariablePropertyArgs(Expression[] args) throws XPathException {
        int len = (args == null) ? 0 : args.length;
        if (len != 2) {
            throw new XPathException(mMsgMgr.getString("XPathModelImpl.getVariablePropertyArgs"));
        }
    }

    /**
     * Validate arguments passed to doMarshal functions.
     */
    private void validateDoMarshalArgs(Expression[] args) throws XPathException {
        int len = (args == null) ? 0 : args.length;
        if (len != 1) {
            throw new XPathException(mMsgMgr.getString("XPathModelImpl.doMarshalMissingArgs"));
        }
    }
    /**
     * Validate arguments passed to doUnMarshal functions.
     */
    private void validateDoUnMarshalArgs(Expression[] args) throws XPathException {
        int len = (args == null) ? 0 : args.length;
        if (len != 1) {
            throw new XPathException(mMsgMgr.getString("XPathModelImpl.doUnMarshalMissingArgs"));
        }
    }
    /**
    ...
    static boolean isValidFunction(String functionName) {
        // strip out the prefix and compare. To be accurate we need to do compare NS
        ...
        for (int i = 0; i < VALID_FUNCTION_NAMES.length; i++) {
            if (name.equals(VALID_FUNCTION_NAMES[i])) {
                return true;
            }
        }
        ...
        //No mechanisum to verify POJO fucntion call is valid
        return true;
    }
}
