...
/**
 * Implements endpoint lifecycle standards described here: <a 
 * href="http://wiki.open-esb.java.net/Wiki.jsp?page=LifecycleHandlingOfComponentsAndServiceUnits">
 * LifecycleHandlingOfComponentsAndServiceUnits</a>.
 * 
 * @author Kevan Simpson
 */
public class DefaultEndpointLifeCycle implements EndpointLifeCycle {
    private EndpointManager mManager;
    private ComponentContext mCtx;
    private Logger mLog;
    
    public DefaultEndpointLifeCycle(ComponentContext ctx) {
        mCtx = ctx;
        mLog = Util.getLogger(ctx, EndpointLifeCycle.class.getName());
    }
    ...
    public void startProvisioning(ServiceUnit srvcUnit) throws DeploymentException {
        ...
        try {
            for (EndpointInfo info : endpts) {
                ...
                // create endpoint
                ept = getEndpointManager().createEndpoint(info, srvcUnit);
    ...
    public void stopProvisioning(ServiceUnit srvcUnit) throws DeploymentException {
        ...
        for (EndpointInfo info : endpts) {
            try {
                Endpoint ept = getEndpointManager().removeEndpoint(info);
                if (info.isProvides()) {
	                ept.stop();
