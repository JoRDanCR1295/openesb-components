...
public class Attribute implements Cloneable {
    ...
    private static final Messages mMessages = Messages.getMessages(Attribute.class);
    ...
    public Attribute(Attribute src) throws BaseException {
        if (src == null) {
            String msg=mMessages.getString("ETLSE-E0458.cannot_create_new_AttributeInstance");
            throw new IllegalArgumentException(msg+ src);
        }
    ...
    public Attribute(String name, Object value) {
        if (StringUtil.isNullString(name)) {
            throw new IllegalArgumentException(mMessages.getString("ETLSE-E0459.empty_String_for_name"));
        }

        if (value == null) {
            throw new IllegalArgumentException(mMessages.getString("ETLSE-E0460.null_ObjectRef_for_value"));
        }

        attributeType = value.getClass().getName();
        if (!Attribute.isValidType(attributeType)) {
            throw new IllegalArgumentException(mMessages.getString("ETLSE-E0461.Invalid_AttributeType")+ attributeType);
        }
    ...
    public Object clone() throws CloneNotSupportedException {
        ...
        try {
            ...
            throw new CloneNotSupportedException(mMessages.getString("ETLSE-E0462.cannot_create_clone")+ this.toString());
        }
        return attr;
    }
    ...
    private void createValueFor(String value, String typeName) throws BaseException {
        if (StringUtil.isNullString(typeName)) {
            throw new IllegalArgumentException(mMessages.getString("ETLSE-E0463.empty_String_for_typeName"));
        }

        if (!isValidType(typeName)) {
            throw new IllegalArgumentException(mMessages.getString("ETLSE-E0461.Invalid_AttributeType")+ typeName);
        }
        ...
        if (typeName.equals(List.class.getName()) || typeName.equals(ArrayList.class.getName())) {
            ...
            try {
                ...
                throw new BaseException(mMessages.getString("ETLSE-E0464.Type_doesnot_have_constructor") + typeName, e);
            } catch (Exception e) {
                throw new BaseException(mMessages.getString("ETLSE-E0465.Could_not_load_class_for_type")+"'" + attributeType + "'.", e);
            }
        }
    }
}
