...
public class ETLProcessHandler {
	...
	private static transient final Logger mLogger = Logger.getLogger(ETLProcessHandler.class.getName());
        private static final Messages mMessages = Messages.getMessages(ETLProcessHandler.class);
	...
	public String startProcess(ETLEngine etlEngine) {
		...
		StringBuffer sb = createOutputMessageString(etlEngine);
                mLogger.log(Level.INFO,mMessages.getString("ETLSE-I0102.Response")+ sb.toString());
		return sb.toString();
	}
	...
	private void setOutputParams(ETLEngine etlEngine) {
		...
		while (it.hasNext()) {
			...
			if (key.equalsIgnoreCase(STATUS)) {
                                mLogger.log(Level.INFO,mMessages.getString("ETLSE-I0103.Setting_outputValue",new String[]{key,status}));
				outputParams.setStatus(status);
			} else if (key.equalsIgnoreCase(START_TIME)) {
				mLogger.log(Level.INFO,mMessages.getString("ETLSE-I0103.Setting_outputValue",new String[]{key,getTimeStampString(starttime)}));
				outputParams.setStartTime(getTimeStampString(starttime));
			} else if (key.equalsIgnoreCase(END_TIME)) {
				mLogger.log(Level.INFO,mMessages.getString("ETLSE-I0103.Setting_outputValue",new String[]{key,getTimeStampString(endtime)}));
				...
				if (context != null) {
					...
					mLogger.log(Level.INFO,mMessages.getString("ETLSE-I0103.Setting_outputValue",new Object[]{key,ct}));
