...
public class ETLSEComponent implements Component {
	...
	private ServiceUnitManager mServiceUnitManager;
        ...
        private EtlMapEntryTable etlMapEntryTable;
	...
	public ServiceUnitManager getServiceUnitManager() {
        ...
        public EtlMapEntryTable getETLMapEntryTable() {
            return this.etlMapEntryTable;
        }
        
        public void setETLMapEntryTable(EtlMapEntryTable etlMapEntryTable) {
            this.etlMapEntryTable = etlMapEntryTable;
        }
	 ...
	 * Note that the search for MashupMapEntry is by operation name execute
         * This will work for current scenario where execute is the only operation 
         * supported. In Future releases, we have to consider extending it to 
         * operation agnostic search. 
         * 
	 * @param endpoint -
	...
	public org.w3c.dom.Document getServiceDescription(ServiceEndpoint endpoint) {
            org.w3c.dom.Document wsdlDocument = null;
	    QName operation = new QName("execute");
            QName service = endpoint.getServiceName();

 	    EtlMapEntry etlMapEntry = this.etlMapEntryTable.findETLEntry(operation,service);
            Definition def = etlMapEntry.getWsdl();
            WSDLWriter writer = new WSDLWriterImpl();
            try {
                wsdlDocument = writer.getDocument(def);
            } catch (WSDLException ex) {
                Logger.getLogger(ETLSEComponent.class.getName()).log(Level.SEVERE, null, ex);
            }
            return wsdlDocument;
	}
