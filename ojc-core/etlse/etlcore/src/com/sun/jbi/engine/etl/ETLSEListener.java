...
public class ETLSEListener implements ETLEngineListener {
	...
	private static final Messages mMessages = Messages.getMessages(ETLSEListener.class);
	ETLEngine engine;
	...
	public void executionPerformed(ETLEngineExecEvent event) {
		if ((event.getStatus() == ETLEngine.STATUS_COLLAB_COMPLETED)
			...
			try {
				...
				mLogger.log(Level.INFO,(event.getStatus() == ETLEngine.STATUS_COLLAB_COMPLETED) ? "MSG_executed_success"
						: "MSG_executed_errors");
			} catch (Exception ex) {
				;
			} finally {
