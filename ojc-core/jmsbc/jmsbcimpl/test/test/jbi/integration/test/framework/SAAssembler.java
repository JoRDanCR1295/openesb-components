...
public class SAAssembler {
	...
	public String assemble(String workingDir) throws IOException{
		...
		{
			//Generate OSGI bundle MANIFEST.MF
			StringBuilder buf = new StringBuilder();
			buf.append("Bundle-Version: 1.0\r\n");
			buf.append("Bundle-ManifestVersion: 2\r\n");
			buf.append("Bundle-SymbolicName: ").append(name).append("\r\n");
			buf.append("Dynamic-Update: false\r\n");
			buf.append("DynamicImport-Package: *\r\n");

			FileOutputStream out = new FileOutputStream(meta + File.separator + "MANIFEST.MF");
			out.write(buf.toString().getBytes());
			out.close();
		}
