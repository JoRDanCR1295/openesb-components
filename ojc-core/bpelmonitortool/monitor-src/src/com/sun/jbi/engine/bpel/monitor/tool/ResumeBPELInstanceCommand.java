...
/*
<<<<<<< ResumeBPELInstanceCommand.java
 * @(#)$Id: ResumeBPELInstanceCommand.java,v 1.10 2008/05/07 19:12:07 mei_wu Exp $
=======
 * @(#)$Id: ResumeBPELInstanceCommand.java,v 1.10 2008/05/07 19:12:07 mei_wu Exp $
>>>>>>> 1.8.4.1
 *
...
public class ResumeBPELInstanceCommand extends Command {


	private static final String SUCCESS = "BPCOR-5003: Successfully resumed instance:{0}";

	private static final String FAIL = "BPCOR-5004: Fail to resume instance:{0}";
    
    private static final String TOTAL = "BPCOR-5010: Total {0} instances {1}";    

	private static final String NO_INSTANCE = "BPCOR-6006: The instance :{0} does not exist or is not in suspended state, please check the instance status";
	...
	public ResumeBPELInstanceCommand(String name, CommandContext context) {
		super(name, context);
		...
		if (mInstances.size() == 0) {
			try {
				List<String> suspended = mContext.getResource()
						.getMBpelService().resumeAllInstance(processId, mContext.getResource().getTargetName());
				...
				if (suspended != null && suspended.size() > 0) {
                    ...
                    result.append(I18n.loc(TOTAL, suspended.size(), "RESUMED"));                    
				}
			} catch (Exception e) {
			...
			for (String instanceid : mInstances) {
				try {
					// BPELManagementAPIFactory.getAPI().resumeInstance(instanceId,
					// mContext.getResource().getMbeanConn());
					boolean done = mContext.getResource().getMBpelService()
							.resumeInstance(instanceid, mContext.getResource().getTargetName());
					if (done) {
						result.append(I18n.loc(SUCCESS, instanceId));
						result.append("\n");
					} else {
