...
/*
<<<<<<< TerminateBPELInstanceCommand.java
 * @(#)$Id: TerminateBPELInstanceCommand.java,v 1.11 2008/05/07 19:12:08 mei_wu Exp $
=======
 * @(#)$Id: TerminateBPELInstanceCommand.java,v 1.11 2008/05/07 19:12:08 mei_wu Exp $
>>>>>>> 1.9.4.1
 *
...
public class TerminateBPELInstanceCommand extends Command {

	private static final String SUCCESS = "BPCOR-5008: Successfully terminate instance:{0}";

	private static final String FAIL = "BPCOR-5009: Fail to terminate instance:{0}";
	...
	public TerminateBPELInstanceCommand(String name, CommandContext context) {
		super(name, context);
		...
		if (mInstances.size() == 0) {
			try {
				List<String> suspended = mContext.getResource()
						.getMBpelService().terminateAllInstance(processId, mContext.getResource().getTargetName());
				...
				if (suspended != null && suspended.size() > 0) {
                    ...
                    result.append(I18n.loc(TOTAL, suspended.size(), "TERMINATED"));                                
				}
			} catch (Exception e) {
			...
			for (String instanceid : mInstances) {
				try {
					boolean done = mContext.getResource().getMBpelService()
							.terminateInstance(instanceid, mContext.getResource().getTargetName());
					if (done) {
						result.append(I18n.loc(SUCCESS, instanceid));
						result.append("\n");
					} else {
