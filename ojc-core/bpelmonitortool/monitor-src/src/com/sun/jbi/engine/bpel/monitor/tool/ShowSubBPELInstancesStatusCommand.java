...
/*
 * @(#)$Id: ShowSubBPELInstancesStatusCommand.java,v 1.5 2008/05/07 19:12:07 mei_wu Exp $
 *
...
public class ShowSubBPELInstancesStatusCommand extends Command {

	private static final String INVALID_STATUS = "The status :{0} is not valid";

	public ShowSubBPELInstancesStatusCommand(String name, CommandContext context) {
		super(name, context);
	...
	public CommandContext execute() throws Exception {
		...
		try {
			List<BPInstanceInfo> bpifs = mContext.getResource()
					.getMBpelService().getInvokeeInstance(instid, actId, mContext.getResource().getTargetName());
