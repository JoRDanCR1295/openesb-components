...
/*
 * @(#)HttpSoapPerformanceMeasurement.java 
 *
 ...
 */
public class FileBCPerformanceMeasurement implements PerformanceMeasurement {

    public static final String FILE_BC_SOURCE_FILTER = ".*filebc.*";
    ...
    public TabularData getPerformanceInstrumentationMeasurement(String endpoint) throws OpenDataException, MBeanException {
        ...
        gr.addPattern(FILE_BC_SOURCE_FILTER, endpoint, getPerfCatFilter());
        return gr.fetchData();
    }
    ...
    private String getPerfCatFilter() {
        ...
        for (int i = 0; i < FileBindingLifeCycle.FILEBC_PERF_CATEGORIES.length; i++) {
            perfCatFilter.append(FileBindingLifeCycle.FILEBC_PERF_CATEGORIES[i]);
            if (i < FileBindingLifeCycle.FILEBC_PERF_CATEGORIES.length - 1) {
