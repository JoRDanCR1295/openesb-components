...
/**
 * Access to JCS properties.  This checks the system properties first,
 * then the definitions in the user's own ~/.jcsrc file (if any), and
...
public class JCSProperties
{
	...
	new File(System.getProperty("user.home"), ".jcsrc");
	...
	{
	    // There is no ".jcsrc" file at all; perfectly legal.
	    //- System.out.println("no init file \"" + jcsFile.getPath() + '"');
	}
	catch (java.io.IOException io)
	{
	    // There is a ".jcsrc" file, but it's broken or inaccessible.
	...
	preProp.setProperty("JGen.antlr", "com.sun.stc.antlr");
    }

    /**
     * Gets a JCS property (cached).
     *
    ...
    /**
     * Gets a JCS property (cached), or default.
     *
     * @param key  the property name
    ...
    /**
     * Get a JCS property; optionally make sure the initialization file
	...
	throw new RuntimeException("JCS property [" + key +
	    "] has illegal value \"" + res + "\"");
    }

    /**
     * Get a JCS integer property (cached).
     * If the property is not defined, return the default supplied here.
     ...
     * "-j" will display the list of properties read from the ".jcsrc"
     * file.
     *
	...
	for (int i = 0; i < args.length; i ++)
	{
	    ...
	    if (args[i].equals("-j"))
	    {
	        // Option -j: print the properties in the ".jcsrc" file.
