...
/**
 * This class provides HL7 specific implementation for escaping/unescaping
 * character sequences.
 * 
 * @author Jun Xu
 * @since 6.0
...
public final class HL7CharEscapeCoder {
    ...
    private static final Logger mLog =
        Logger.getLogger(HL7CharEscapeCoder.class.getName());

    // constants for state of processing
    ...
    // constants for state of escaping
    ...
    public HL7CharEscapeCoder(char fieldSep, char compoSep, char subCompoSep,
    ...
    public void escape(char[] srcchars, int srcoffset, int count) {
        if (mLog.isLoggable(Level.FINEST)) {
            StringBuilder sb = new StringBuilder(
                "Data in Character Array (char[]) before escaping the characters: ");
            sb.append(new String(srcchars));
		...
		if (mResData != null && mLog.isLoggable(Level.FINEST)) {
			StringBuilder sb = new StringBuilder(
                "Data in Character Array (char[]) after escaping the characters: ");
			sb.append(new String(mResData));
			mLog.finest(sb.toString());
		}
    }
...
whileloop:
        while (i < end) {
            if (mEscapeState == ESCAPE_UNDET) {
                ...
                if (i - 1 > mPos) {
                    ...
                    System.arraycopy(mData, mPos, mResData, mResPos, i - 1 - mPos);
                    mResPos += (i - 1 - mPos);
                    mPos = i - 1;
                    copiedData = true;
                }
                ...
                if (mEscapeState == ESCAPE_NONE) {
                    ...
                    switch (mEscapeState) {
                        ...
                        case ESCAPE_HEX:
                            if (i > mPos + 2) {
                                String hexVal = new String(mData, mPos + 2, i - mPos - 2);
                                char[] chars = Character.toChars(Integer.parseInt(hexVal, 16));
                                if (chars.length == 1) {
                                    mResData[mResPos++] = chars[0];
                                } else {
                                    System.arraycopy(chars, 0, mResData, mResPos, chars.length);
        ...
        if (mEscapeState == ESCAPE_NONE) {
            if (mPos < i) {
                System.arraycopy(mData, mPos, mResData, mResPos, i - mPos);
                mResPos += (i - mPos);
                mPos = i;
            }
