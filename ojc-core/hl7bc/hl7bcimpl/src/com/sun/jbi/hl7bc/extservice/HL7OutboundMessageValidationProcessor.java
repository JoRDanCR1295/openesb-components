...
/*
 * @(#)HL7OutboundMessageValidationProcessor.java 
 *
...
/**
 * This class validates the HL7 Outbound message
 * 
 * @author Raghunadh
 */

public class HL7OutboundMessageValidationProcessor implements HL7Constants {

    private String logMsg;

    private static final Logger mLog = Logger.getLogger(HL7OutboundMessageValidationProcessor.class.getName());
    ...
    public ValidationInfo messageValidationProcess(Endpoint destination, NormalizedMessage normalizedMsg)
        ...
        try {
            if (validateMSH) {
                getMSHFieldValues((DOMSource) source);
                validateHL7Message(protocolProperties, normalizedMsg, mValidationInfo);
            }
        } catch (Exception ex) {
            //ignore exception
        }
    ...
    private void validateHL7Message(HL7ProtocolProperties protocolProperties,
                                    NormalizedMessage normalizedMsg,
                                    ValidationInfo validationInfo) throws Exception {

        if (mLog.isLoggable(Level.INFO)) {
            mLog.log(Level.INFO, I18n.msg("I0171: Validating the HL7 message in Outbound"));
        }
