...
public class EndpointConfigurationSUDescriptor implements EndpointConfiguration {
	...
	private ArrayList<EndpointData> services;

	public EndpointConfigurationSUDescriptor(String rootDir)
			throws ConfigurationException {

		services = new ArrayList<EndpointData>();
	...
	private void addEndpoints(EndpointIdentifier[] endpoints) {
		if (endpoints != null) {
			for (int i = 0; i < endpoints.length; i++) {
				...
				int direction = endpoint.isProvider() ? EndpointType.OUTBOUND
						: EndpointType.INBOUND;
                
                String appconfName = endpoint.getApplicationConfigurationName();

				addEndpoint(endpoint.getInterfaceName().toString(), endpoint
						.getServiceName().toString(), endpoint
						.getEndpointName(), appconfName, direction);
			}
		}
	}
	...
	public void addEndpoint(String interfaceName, String service,
			String endPoint,String appconfName, int direction) {

		services.add(new EndpointDataImpl(interfaceName, service, endPoint, appconfName,
				direction));
	}
