...
public class UnionMySql implements UnionDb {
    private static final Messages mMessages = Messages.getMessages(UnionMySql.class);
    
    private MySqlSpecial mMySqlSpecial;
    
    public UnionMySql(MySqlSpecial mySqlSpecial) {
    ...
    public PreparedStatement[] createOperateStatements(Connection con, Union op) throws Exception {
        ...
        sb.append(COL_TIMESTAMP + "," + COL_TAG + ") SELECT DISTINCT ");
        for (int i = 0; i < columnNames.length; i++) {
            sb.append(columnNames[i] + ",");
        }
