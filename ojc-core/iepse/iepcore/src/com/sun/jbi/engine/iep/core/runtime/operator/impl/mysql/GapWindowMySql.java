 ...
 * @author Bing Lu
 */
public class GapWindowMySql implements GapWindowDb {
    private static final Messages mMessages = Messages.getMessages(GapWindowMySql.class);
    
    private MySqlSpecial mMySqlSpecial;
    
    public GapWindowMySql(MySqlSpecial mysqlSpecial) {
    ...
    public PreparedStatement[] createOperateStatements(Connection con, GapWindow op) throws Exception {
        ...
        sb.append(attribute + ", " + COL_TIMESTAMP + ", " + COL_TAG + ") ");
        sb.append("(SELECT ");
        for (int i = 0; i < attrName.length; i++) {
