 ...
 * @author Bing Lu
 */
public class PartitionedWindowMySql implements PartitionedWindowDb {
    private static final Messages mMessages = Messages.getMessages(PartitionedWindowMySql.class);
    
    private MySqlSpecial mMySqlSpecial;
    
    public PartitionedWindowMySql(MySqlSpecial mySqlSpecial) {
    ...
    public PreparedStatement[] createOperateStatements(Connection con, PartitionedWindow op) throws Exception {
        ...
        sb = new StringBuffer();
        sb.append("UPDATE ");
        sb.append(outputTableName + " t1, ");
        sb.append("(SELECT t2." + COL_SEQID + " AS " + COL_SEQID + ", COUNT(*) AS " + COL_PSEQID);
        sb.append(" FROM " + outputTableName + " t2, " + outputTableName + " t3");
        sb.append(" WHERE t2." + COL_TAG + " = '+' AND t3." + COL_TAG + " = '+' AND ");
        for (int i = 0; i < attrCnt; i++) {
            sb.append("t3." + attrName[i] + " = t2." + attrName[i] + " AND ");
        }
        sb.append("t3." + COL_SEQID + " <= t2." + COL_SEQID);
        sb.append(" GROUP BY t2." + COL_SEQID + ") t4 ");

        sb.append("Set t1." + COL_PSEQID + " = t4." + COL_PSEQID);
        sb.append(" WHERE t1." + COL_SEQID + " = t4." + COL_SEQID);
        sqlStr = sb.toString();
        if (mMessages.isLoggable(Level.FINE)) {
