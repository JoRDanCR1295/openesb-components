...
/*
 * @(#)TupleBasedAggregatorMySql.java 
 *
 ...
 * @author Bing Lu
 */
public class TupleBasedAggregatorMySql implements TupleBasedAggregatorDb {
    private static final Messages mMessages = Messages.getMessages(TupleBasedAggregatorMySql.class);
    
    private MySqlSpecial mMySqlSpecial;
    
    public TupleBasedAggregatorMySql(MySqlSpecial mysqlSpecial) {
    ...
    public PreparedStatement[] createOperateStatements(Connection con, TupleBasedAggregator op) throws Exception {
        ...
        //    SELECT toColumn1, toColumn2, Seqid, Timestamp FROM (
        //       SELECT fromColumn1 as toColumn1, fromColumn2 as toColumn2, S2.MAX(Seqid), 
        //       (SELECT MAX(Timestamp) from S1 where Seqid=seqidToProcess) FROM fromClause(T1, T2, S1, S2)
        //       WHERE prevSeqidToProcess < S1.Seqid AND S1.Seqid <= seqidToProcess AND whereClause(T1, T2, S1, S2)
        ...
        sb.append("(SELECT MAX(" + inputTableName + "." + COL_TIMESTAMP + ") FROM " + inputTableName 
        		+ " WHERE " + inputTableName + "." + COL_SEQID + " = ?) AS " + COL_TIMESTAMP);
        sb.append(" FROM ");
        ...
        try {
            ...
            if ((op.getStart() + op.getIncrement() - 1) <= seqidToProcess && op.getPrevSeqidToProcess() < seqidToProcess) {
                for (long i = 1 + (op.getPrevSeqidToProcess() - op.getStart() + 1)/op.getIncrement(), I = (seqidToProcess - op.getStart() + 1)/op.getIncrement(); i <= I; i++) {
                    ...
                    op.getOperateStatement(1).setLong(2, op.getStart() + i*op.getIncrement() - op.getSize() - 1);
                    op.getOperateStatement(1).setLong(3, op.getStart() + i*op.getIncrement() - 1);
