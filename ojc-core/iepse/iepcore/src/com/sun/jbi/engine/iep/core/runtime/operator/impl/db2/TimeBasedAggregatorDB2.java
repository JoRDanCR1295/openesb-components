...
/*
 * @(#)TimeBasedAggregatorOracle.java 
 *
 ...
 * @author Bing Lu
 * 
 * Fortent : Based on TimeBasedAggregatorOracle.java
 */
public class TimeBasedAggregatorDB2 implements TimeBasedAggregatorDb {
    private static final Messages mMessages = Messages.getMessages(TimeBasedAggregatorDB2.class);
    
	private DB2Special mDB2Special;

	public TimeBasedAggregatorDB2(DB2Special dB2Special) {
    ...
    public void executeOperateStatements(TimeBasedAggregator op, Timestamp prevT, Timestamp curT) throws Exception {
        ...
        try {
            ...
            if ((rs.next()) && ((minTv = rs.getTimestamp(1)) != null)) {
                long minLv = minTv.getTime();
                
                // performance improvement
                minLv = op.getStart() + ((minLv - op.getStart())/op.getSize())*op.getSize();
                ...
                curLv = op.getStart() + ((curLv - op.getStart())/op.getSize())*op.getSize();
