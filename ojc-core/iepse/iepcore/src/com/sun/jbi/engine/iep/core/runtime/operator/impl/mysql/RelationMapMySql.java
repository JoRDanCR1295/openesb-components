 ...
 * @author Bing Lu
 */
public class RelationMapMySql implements RelationMapDb {
    private static final Messages mMessages = Messages.getMessages(RelationMapMySql.class);
    
    private MySqlSpecial mMySqlSpecial;
    
    public RelationMapMySql(MySqlSpecial mySqlSpecial) {
    ...
    public PreparedStatement[] createOperateStatements(Connection con, RelationMap op) throws Exception {
        ...
        for (int i = 0; i < inputCount; i++) {
            ...
            for (int j = 0; j < inputCount; j++) {
                ...
                if (j != i) {
                    sb.append("NOT EXISTS (SELECT 'x' FROM " + inputTableName[j] + " r");
                    sb.append(" WHERE r." + COL_SEQID + " = " + inputTableName[j] + "." + COL_SEQID + " AND ");
        ...
        for (int i = 0; i < inputCount; i++) {
            ...
            sb.append(" FROM ");
            sb.append(inputTableName[i] + " " + t[i] + ", ");
            sb.append(outputTableName + " r");
            sb.append(" WHERE ");
