 ...
 * @author Bing Lu
 */
public class TupleBasedAggregatorOracle implements TupleBasedAggregatorDb {
    private static final Messages mMessages = Messages.getMessages(TupleBasedAggregatorOracle.class);
    
    private OracleSpecial mOracleSpecial;
    
    public TupleBasedAggregatorOracle(OracleSpecial oracleSpecial) {
    ...
    public PreparedStatement[] createOperateStatements(Connection con, TupleBasedAggregator op) throws Exception {
        ...
        //       SELECT fromColumn1 as toColumn1, fromColumn2 as toColumn2, 
        //       (SELECT MAX(Timestamp) from S1 where Seqid=seqidToProcess) FROM fromClause(T1, T2, S1)
