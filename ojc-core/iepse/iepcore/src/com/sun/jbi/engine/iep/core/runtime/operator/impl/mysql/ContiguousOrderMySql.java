 ...
 * @author Bing Lu
 */
public class ContiguousOrderMySql implements ContiguousOrderDb {
    private static final Messages mMessages = Messages.getMessages(ContiguousOrderMySql.class);
    
    private MySqlSpecial mMySqlSpecial;
    
    public ContiguousOrderMySql(MySqlSpecial mysqlSpecial) {
    ...
    public PreparedStatement[] createOperateStatements(Connection con, ContiguousOrder op) throws Exception {
        ...
        sb.append(" AND w." + attribute + " > " + start + ")"); // end of INSERT INTO
        sqlStr = sb.toString();
        if (mMessages.isLoggable(Level.FINE)) {
