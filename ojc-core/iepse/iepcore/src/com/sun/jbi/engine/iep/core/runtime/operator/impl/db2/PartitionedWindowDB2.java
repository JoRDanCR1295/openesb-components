 ...
 * @author Bing Lu
 * 
 * Fortent : Based on PartitionedWindowOracle.java
 */
public class PartitionedWindowDB2 implements PartitionedWindowDb {
    private static final Messages mMessages = Messages.getMessages(PartitionedWindowDB2.class);
    
    private DB2Special mDB2Special;
    
    public PartitionedWindowDB2(DB2Special dB2Special) {
    ...
    public PreparedStatement[] createOperateStatements(Connection con, PartitionedWindow op) throws Exception {
        ...
        sb.append("t2." + COL_PSEQID + " = t1." + COL_PSEQID + " + " + size + " AND ");
        sb.append("NOT EXISTS (SELECT 'x' FROM " + outputTableName + " t3 WHERE ");
        sb.append("t3." + COL_SEQID + " = t1." + COL_SEQID + " AND t3." + COL_TAG + " = '-')");
        sqlStr = sb.toString();
        if (mMessages.isLoggable(Level.FINE)) {
