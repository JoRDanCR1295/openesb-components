 ...
 * @author Bing Lu
 */
public class RelationStreamDerby implements RelationStreamDb {
    private static final Messages mMessages = Messages.getMessages(RelationStreamDerby.class);
    
    private DerbySpecial mDerbySpecial;
    
    public RelationStreamDerby(DerbySpecial derbySpecial) {
        mDerbySpecial = derbySpecial;
    }

    public void createStoredProcedures(Connection con) {
        StringBuffer sb = new StringBuffer();
        sb.append("CREATE PROCEDURE relationStreamOperate (");
        ...
        sb.append(" EXTERNAL NAME 'com.sun.jbi.engine.iep.core.derby.RelationStream.operate'");
        ...
        try {
            ...
            mMessages.log(Level.SEVERE, "DerbySpecial.Fail_to_create_procedure",  "relationStreamOperate", e);
        } finally {
            Util.close(s);
        }
        return;
    ...
    public void dropStoredProcedures(Connection con) {
        ...
        try {
            s = con.createStatement();
            s.execute("DROP PROCEDURE relationStreamOperate");
        } catch (Exception e) {
            mMessages.log(Level.SEVERE, "DerbySpecial.Fail_to_drop_procedure", "relationStreamOperate", e);
        } finally {
            Util.close(s);
        }
    }
    ...
    public PreparedStatement[] createOperateStatements(Connection con, RelationStream op) throws Exception {
        ...
        // {CALL relationStreamOperate('S1', 'R1', 'colNames', ?, ?)}
        String colNames = mDerbySpecial.getString(inputSchema.getColumnNames(), DELIM);
        StringBuffer sb = new StringBuffer();
        sb.append("{CALL relationStreamOperate(");
