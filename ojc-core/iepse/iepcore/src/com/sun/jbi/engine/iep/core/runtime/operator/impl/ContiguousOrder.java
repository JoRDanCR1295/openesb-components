 ...
 * @author Bing Lu
 */
public class ContiguousOrder extends AbstractOperator {
    private static final Messages mMessages = Messages.getMessages(ContiguousOrder.class);
    ...
    public ContiguousOrder(Map prop) throws Exception {
        initialize(prop);
    ...
    private void createTable(Connection con) throws Exception {
        ...
        try {
            ...
            for (int i = 0, I = mAttributeList.size(); i < I; i++) {
                String columnName = (String)mAttributeList.get(i);
    ...
    protected void createOperateStmt(Connection con) throws Exception {
        ...
        mOperateStmt = mDbSpecial.getContiguousOrderDb().createOperateStatements(con, this);
    }
    ...
    protected void executeOperateStmt(Timestamp prevT, Timestamp curT) throws Exception {
        ...
        mOperateStmt[4].setTimestamp(1, curT);
        mOperateStmt[4].setTimestamp(2, prevT);
        mOperateStmt[4].setTimestamp(3, prevT);
        mOperateStmt[4].executeUpdate();
        
        mOperateStmt[5].executeUpdate();

        mOperateStmt[6].setTimestamp(1, curT);
        ResultSet rs = mOperateStmt[6].executeQuery();
        if (rs.next()) {
