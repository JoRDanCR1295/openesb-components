 ...
 *
 */
public class EhloState extends DefaultStateImpl {

    // My content
    private String mDomainName = "";

    public EhloState(final StateFactory factory, final State parent) {
    ...
    public boolean parse(final String content) throws InvalidParseException {
        if (content.toUpperCase().startsWith("EHLO")) {
            final int space = content.indexOf(' ');
            if (space == -1) {
                throw new InvalidParseException("550 5.0.0 EHLO requires domain address");
            }
