...
public class BPVisualizerBean extends ChartingConstants {
    ...
    static {
        ...
        BpelsePluginMessages.getString("bpvisualizer_Bpel_select_su");
        SE_TITLE_STRING  = 
        BpelsePluginMessages.getString("bpvisualizer_Bpel_se_title");
        SUS_TITLE_STRING  = 
        BpelsePluginMessages.getString("bpvisualizer_Bpel_sus_title");
        SU_OPTION_DROP_DOWN_LABEL = 
            BpelsePluginMessages.getString("bpvisualizer_suinstances_label");
        CHART_OPTION_DROP_DOWN_LABEL = 
            BpelsePluginMessages.getString("bpvisualizer_chartoptions_label");
    }
    ...
    private boolean customPlotRequested = false;
    ...
    public String getViewActionText() {
        if(aggregatedView) {
            return BpelsePluginMessages.getString("bpvisualizer_instanceview_label");
        }
        return BpelsePluginMessages.getString("bpvisualizer_aggregatedview_label");
   }

   public String getInLineHelpText() {
        if(isAggregatedView()) {
            return BpelsePluginMessages.getString("bpvisualizer_Bpel_select_se_label");
        }
        return BpelsePluginMessages.getString("bpvisualizer_Bpel_select_su_label");
   }
             ...
             if(serviceUnitInstanceSelected != null && 
                 ...
                 if(ChartSelectionMenuSelectedOption.equals(BP_SERVICE_UNIT_STATUS)) {
                     title = serviceUnitInstanceSelected + 
                             BpelsePluginMessages.getString("bpvisualizer_Bpel_chart_title_status_su");
                 } else  if(ChartSelectionMenuSelectedOption.equals(BP_SERVICE_UNIT_CUSTOM)) {
                      title = serviceUnitInstanceSelected + 
                             BpelsePluginMessages.getString("bpvisualizer_Bpel_chart_title_custom_su");
                 }
             }
          } else {
             if(ChartSelectionMenuSelectedOption.equals(BPS_AGGREGATED_STATUS)) {
                 title = BpelsePluginMessages.getString("bpvisualizer_Bpel_chart_title_status_all");
             }
    ...
    public String getChartXLabel() {
        ...
        if(aggregatedView == false) {
           if(serviceUnitInstanceSelected != null && 
                   serviceUnitInstanceSelected.length() > 0) {
               if(ChartSelectionMenuSelectedOption.equals(BP_SERVICE_UNIT_STATUS)) {
                   title =  BpelsePluginMessages.getString("bpvisualizer_bpel_name");
               } else  if(ChartSelectionMenuSelectedOption.equals(BP_SERVICE_UNIT_CUSTOM)) {
                   // delegate to the custom bean
                   javax.faces.context.FacesContext context =
                       javax.faces.context.FacesContext.getCurrentInstance();
               
                   javax.faces.el.ValueBinding valueBinding =
                       context.getApplication().createValueBinding(BPCustomChartBean.BEAN_NAME);
               
                   BPCustomChartBean customBean =
                       (BPCustomChartBean) valueBinding.getValue(context);
                   title =  customBean.getChartXLabel();
               }
           }
        } else {
           if(ChartSelectionMenuSelectedOption.equals(BPS_AGGREGATED_STATUS)) {
               title = BpelsePluginMessages.getString("bpvisualizer_su_name");
           }
    ...
    public String getChartYLabel() {
        ...
        if(aggregatedView == false) {
           if(serviceUnitInstanceSelected != null && 
                   serviceUnitInstanceSelected.length() > 0) {
               if(ChartSelectionMenuSelectedOption.equals(BP_SERVICE_UNIT_STATUS)) {
                   title = BpelsePluginMessages.getString("bpvisualizer_instances_count");
               } else  if(ChartSelectionMenuSelectedOption.equals(BP_SERVICE_UNIT_CUSTOM)) {
                   // delegate to the custom bean
                   javax.faces.context.FacesContext context =
                       javax.faces.context.FacesContext.getCurrentInstance();
                   ...
                   BPCustomChartBean customBean =
                       (BPCustomChartBean) valueBinding.getValue(context);
                   title = customBean.getChartYLabel();
               }
           }
        } else {
           if(ChartSelectionMenuSelectedOption.equals(BPS_AGGREGATED_STATUS)) {
   ...
   public String getTwoDimensional() {
    ...
    public boolean isChartSelected() {
        if(aggregatedView) {
            return true; // alway render chart
        }
        if(serviceUnitInstanceSelected != null && 
                serviceUnitInstanceSelected.length() > 0) {
            if(ChartSelectionMenuSelectedOption.equals(BP_SERVICE_UNIT_STATUS)) {
        ...
        BPCustomChartBean customBean =
            (BPCustomChartBean) valueBinding.getValue(context);
        return  customBean.isChartSelected();
    }
