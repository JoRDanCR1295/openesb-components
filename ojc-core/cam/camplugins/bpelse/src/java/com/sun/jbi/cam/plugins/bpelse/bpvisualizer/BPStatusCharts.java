...
public final class BPStatusCharts extends ChartSupport {
    ...
    static {
        CATEGORY_BPINSTNACE_DONE = 
                 BpelsePluginMessages.getString("bpvisualizer_BpelInstnace_done");
        CATEGORY_BPINSTNACE_RUNNING  = 
                BpelsePluginMessages.getString("bpvisualizer_BpelInstnace_running");
    }
    ...
    public void setChartTypeOptions(boolean includePieCharts) {
        ...
        setChartTypeMenuSelectedOption(ChartType.Bar.getChartType());
    }
   ...
   public Dataset getChartData() {
       if(aggregatedView && isPieChart()) {
           ...
           return dbHelper.getSUStatusdBarChartData(serviceUnitSelected);
       }
   ...
   private DefaultCategoryDataset getAggregateStatusdBarChartData(List<String> suNameList) {
       List<BPStatusBarChartData> dataList = 
               dbHelper.getAggregateStatusdBarChartData();
       Map<String,List<String>> su2BpelidsMap = new HashMap<String,List<String>>();
       BPELModelProcessor modelProcessor = BPELModelProcessor.getInstance();
       for (String suName : suNameList) {
           try {
               List<String> bpelids = modelProcessor.getBPELIDList(suName);
               su2BpelidsMap.put(suName, bpelids);
           } catch (Exception e) {
            e.printStackTrace();
            return new DefaultCategoryDataset();
           }       
       ...
       DefaultCategoryDataset dataSet = fixAggregateDataSet(dataList,suNameList,su2BpelidsMap);
       return dataSet;
   }

   @SuppressWarnings("unchecked")
   // the data return from the database is group by bpelid
   ...
   private DefaultCategoryDataset getSUStatusdBarChartData(String suName) {
       DefaultCategoryDataset dataSet = 
               dbHelper.getSUStatusdBarChartData(suName);
       return dataSet;
   }
   ...
   private class DBHelper {
     private final static String GET_AGGREGATED_STATUS_QUERY = 
        "select status,count(status) as statecount from STATE group by status";
    private final static String GET_SERVICEUNITS_AGGREGATED_STATUS_QUERY =
     "select BPELID as suid ,status,count(status) as statecount  " +
         "from STATE  group by status,BPELID order by BPELID"; 
    private final static String GET_GIVEN_SERVICEUNIT_AGGREGATED_STATUS_QUERY =
     "select bpelid as suid,status,count(status) as statecount " +
     "from STATE  where bpelid in (%bpid%) group by status,bpelid"; 
   ...
   private  List<BPStatusBarChartData> getAggregateStatusdBarChartData() {
   ...
   private  DefaultCategoryDataset getSUStatusdBarChartData(String suName) {
       ...
       try {
            bpelids = modelProcessor.getBPELIDList(suName);
       } catch (Exception e) {
           e.printStackTrace();
           return dataSet;
       }       
       ...
       int size = bpelids.size();
       for (int index = 0; index < size;index++) {
           idList.append("'" + bpelids.get(index) + "'");
           if(index < size-1) {
               idList.append(","); 
           }
       }
       ...
       String query = GET_GIVEN_SERVICEUNIT_AGGREGATED_STATUS_QUERY.replace("%bpid%",
               idList.toString());
