...
public class IOUtilities {
	...
	private static final java.util.logging.Logger mLog = java.util.logging.Logger
			.getLogger(IOUtilities.class.getName());

	private static final int DEFAULT_BUFFER_SIZE = 1024 * 4;

	// UTF-8
	public static void encode(InputStream input, String srcEncoding,
			OutputStream output, String targetEncoding) throws IOException {
		Reader in = new InputStreamReader(input, srcEncoding);
		Writer out = new OutputStreamWriter(output, targetEncoding);
		copy(in, out);
	}

	// UTF-8
	public static byte[] encode(byte[] srcBuf, String srcEncoding,
	...
	public static InputStream getResourceAsStream(String name) {
		...
		try {
			...
			ret = IOUtilities.class.getClassLoader().getResourceAsStream(name);
			if (ret == null) {
	...
	public static void unzip(String zipFilePath, String targetDirPath)
		...
		for (Enumeration e = zipFile.entries(); e.hasMoreElements();) {
			...
			if (entry.isDirectory()) {
				...
				InputStream input = new BufferedInputStream(zipFile
