...
/*
 * @(#)EngineServiceUnitDescriptorGenerator.java 
 *
 ...
 * 
 */
public class EngineServiceUnitDescriptorGenerator implements Serializable {
	 ...
	 * 
	 */
	public EngineServiceUnitDescriptorGenerator() {
	...
	public String serialize(AspectMap aspectMap)
		...
		for (Aspect aspect : aspectList) {
			...
			namespace = input.getPartnerLinkQName().getNamespaceURI();
			if ((namespace != null)
					&& (namespace.trim().equals("") == false)
					&& (namespaceMapToPrefixMap.containsKey(namespace) == false)) {
				namespaceMapToPrefixMap.put(namespace, "ns" + namespaceIndex);
		...
		buffer.append(">\n");
		buffer.append("    <services binding-component=\"false\">\n");
		...
		for (Consumes consumesElement : consumesList) {
			...
			buffer.append("\" link-type=\"standard");
			buffer.append("\"/>\n");
		}
	...
	public static void main(String[] args) {
		...
		EngineServiceUnitDescriptorGenerator generator = new EngineServiceUnitDescriptorGenerator();

		try {
			parser = AspectMapReader.parseFromFile(uri);
			aspectMap = parser.getAspectMap();
			String content = generator.serialize(aspectMap);
			System.out.println(content);
			String outputFileLocation = "/tmp/aspect_jbi.xml";
