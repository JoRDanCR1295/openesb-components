...
import com.sun.jbi.engine.bpel.BPELSEHelper;
...
import com.sun.jbi.engine.bpel.jbiadaptor.test.atomicbp.ABPEngineChannelSimulatorAdaptor;
...
public class UtilityClass {
    ...
    public void associateABPChannel(final Properties props, final Engine eng, DeploymentBindings deplBindings) 
    throws Exception {
    	final InComingEventModel model = associateChannel(props, deplBindings);
        ABPEngineChannelSimulatorAdaptor channel = 
        	new ABPEngineChannelSimulatorAdaptor(this, props, eng, deplBindings, model.getBPELProcess()); 
        eng.setOutChannel(channel);
    }
...
//            Document expectedDoc = EngineDriver.getDocument(upFileloc);
    ...
    public void initiateBPInstance(final Properties props, 
        ...
        initializeAndSetTransaction(container, engine, model.getBPELProcess());
        setPropagationContext(container);
        System.out.println("sending message:\n"+ inputXML);
        InComingEventKeyImpl event = new InComingEventKeyImpl(model, Event.REQUEST);
        EngineDriver.setInitialMsgExId(messageExchangeId);
        EngineDriver.registerMsgExId(messageExchangeId, oper);
        engine.process(event, container);
    }
    
    private void initializeAndSetTransaction(MessageContainer container, Engine eng, RBPELProcess bpelProcess) {
    	try {
    		BPELProcessManager processManager = eng.getBPELProcessManager(bpelProcess.getBPELId());
    		if (!processManager.isPersistenceEnabled() && (!processManager.isBPAtomic())) {
    			return;
    		}
    ...
    private void setPropagationContext(MessageContainer msgContainer) {
    	
    	try {
    		
    		MessageExchange msgExchange = new DummyMessageExchange(msgContainer.getId());
    		
    		WSMessage wsMessage = (WSMessage) msgContainer.getContent();
            Document jbiMessageDoc = wsMessage.getDocument();
            
            NormalizedMessage nmsg = msgExchange.createMessage();
            nmsg.setContent(new DOMSource(jbiMessageDoc));
            
            msgExchange.setMessage(nmsg, "MESSAGE");
            
            Transaction transaction = msgContainer.getTransaction();
            if (transaction != null) {
            	msgExchange.setProperty(MessageExchange.JTA_TRANSACTION_PROPERTY_NAME, transaction);
            }
            
            PropagationContext propContext = new PropagationContext(msgExchange);
        ...
        EngineChannelSimulatorAdaptor channel = new EngineChannelSimulatorAdaptor() {
            ...
            public Object invoke(MessageContainer msgContainer, RuntimePartnerLink partnerLink, 
                ...
                if (oneWay) {
                    ...
                    mContainer = 
                        MessageContainerFactory.createMessage(mOneWayInvokeMEId, jbiMsg, null, null);
                    initializeAndSetTransaction(mContainer, eng, model.getBPELProcess());
                    InComingEventKeyImpl event = new InComingEventKeyImpl(model, Event.REQUEST);
                    System.out.println("sending one way invoke to engine");
