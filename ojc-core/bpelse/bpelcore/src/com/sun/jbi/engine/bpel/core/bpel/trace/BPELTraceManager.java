...
public class BPELTraceManager {
	...
	/**
	 * Could be called for Virtual activities like CompensateUnit
	 * for which the RActivity is null, hence care is taken to 
	 * avoid trace calls. 
	 * @param trace
	 * @param context
	 */
    ...
    private void logMessages(Iterator<Log> logIter, Context context, String actName, BPELProcessInstance process) {
        ...
        try {
            ...
            contextNDC.add(process.getBPELProcessManager().getServiceAssemblyName());
            ...
            ndc = NDC.enter(contextNDC.toArray());
            ...
            while (logIter.hasNext()) {
                ...
                try {
                    ...
                    if (level.equals(Log.SEVERE)) {
                        BPEL_LOGGER.log(Level.SEVERE, evaluateFrom(from, context));
                    } else if (level.equals(Log.WARNING)) {
                        if (BPEL_LOGGER.isLoggable(Level.WARNING)) {
                            BPEL_LOGGER.log(Level.WARNING, evaluateFrom(from, context));
                        }
                    } else if (level.equals(Log.CONFIG)) {
                        if (BPEL_LOGGER.isLoggable(Level.CONFIG)) {
                            BPEL_LOGGER.log(Level.CONFIG, evaluateFrom(from, context));
                        }
                    } else if (level.equals(Log.INFO)) {
                        if (BPEL_LOGGER.isLoggable(Level.INFO)) {
                            BPEL_LOGGER.log(Level.INFO, evaluateFrom(from, context));
                        }
                    } else if (level.equals(Log.FINE)) {
                        if (BPEL_LOGGER.isLoggable(Level.FINE)) {
                            BPEL_LOGGER.log(Level.FINE, evaluateFrom(from, context));
                        }
                    } else if (level.equals(Log.FINER)) {
                        if (BPEL_LOGGER.isLoggable(Level.FINER)) {
                            BPEL_LOGGER.log(Level.FINER, evaluateFrom(from, context));
                        }
                    } else if (level.equals(Log.FINEST)) {
                        if (BPEL_LOGGER.isLoggable(Level.FINEST)) {
                            BPEL_LOGGER.log(Level.FINEST, evaluateFrom(from, context));
                        }
                    } else {
			...
			try {
				...
				if (level.equals(Alert.CRITICAL)) {	
					alerter.custom(NotificationEvent.EVENT_TYPE_ALERT, NotificationEvent.SEVERITY_TYPE_CRITICAL, 
							evaluateFrom(from, context), COMPONENT_NAME, deploymentName, SERVER_TYPE, 
	 ...
	 */
	private String evaluateFrom(From from, Context context) throws Exception {
        ...
        if (fromVal instanceof Iterator) {
        	...
        	while (sourcePtrIterator.hasNext()) {
				 ...
				 * cases.
				 */
        		stringBuffer.append(currentSourcePtr.getValue().toString());
        	...
        	traceMessage = fromVal.toString();
        } else if (fromVal instanceof Node) {
        	traceMessage = DOMHelper.createXmlString((Node) fromVal);
        } else {
