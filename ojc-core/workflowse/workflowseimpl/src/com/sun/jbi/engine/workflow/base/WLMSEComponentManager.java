...
/*
 * @(#)$Id: WLMSEComponentManager.java,v 1.1 2010/02/15 19:25:07 fkieviet Exp $
 *
...
import com.sun.jbi.component.toolkit.lifecycle.impl.ThreadedExchangeHandler;
...
public class WLMSEComponentManager implements ComponentManager {
    ...
    private WorkflowEngine mEngine = null;
    ...
    protected EndpointManager initEndpointManager() {
        ...
        return new WLMSEEndPointManager (getComponentContext());
    }

    protected ExchangeHandler initExchangeHandler() {
        // TODO Auto-generated method stub
        return  new WLMSEExchangeHandler (getComponentContext());
    }
    ...
    protected Object initRuntimeMBean() throws JBIException {
        return  new WLMSEConfiguration(getComponentContext(), getConfig());
    ...
    protected ServiceUnitManager initServiceUnitManager() {
        WLMSEServiceUnitManager svrManager = new WLMSEServiceUnitManager(getManagerContext()); 
        svrManager.setEngine(mEngine);
        return svrManager;
    }
    ...
    public Document getServiceDescription(ServiceEndpoint endpoint) {
        ...
        WorkflowMapEntry entry = ((WLMSEManagerContext) getManagerContext()).getEntryTable()
        ...
        try {
            ...
            if (wsdlDef != null) {
                ...
                if (task != null) {
                    wsdlDef = com.sun.jbi.workflow.model.impl.Util.getWSDLDefinition(task);
                    if (wsdlDef != null) {
                        WSDLMergeUtil.mergeWSDL(newDef, wsdlDef);
                        wsdlDefDoc = writer.getDocument(newDef);
                        wsdlDefDoc.setDocumentURI(newDef.getDocumentBaseURI());
    ...
    public void init(ComponentContext ctx) throws JBIException {
        ...
        mComponentCtx = ctx;
        Logger logger = Util.getLogger(ctx, this.getClass().getName());
        ...
        mManagerCtx = new WLMSEManagerContext(
                ctx, emgr, handler, getConfig(),
                new CustomClassLoaderUtil(getClass().getClassLoader()), mWorkflowMapEntryTable);        

        emgr.setContext(mManagerCtx);
        handler.setContext(mManagerCtx);
        
        mEngine = new WorkflowEngine();
        mEngine.init();        
        WLMSEInOutProvider inoutprovider = new WLMSEInOutProvider (mEngine, (WLMSEManagerContext) mManagerCtx);
        WLMSEInOnlyConsumer inOnlyConsumer = new WLMSEInOnlyConsumer (mEngine, (WLMSEManagerContext) mManagerCtx);
         
        ((WLMSEExchangeHandler) handler).setInOnlyConsumer (inOnlyConsumer);
        ((WLMSEExchangeHandler) handler).setInOutProvider(inoutprovider);
        ...
        initializeContext(getComponentContext());
        // send alert
    ...
    private void initializeContext(ComponentContext context)
        ...
        try {
            ...
            engineContext.setConfig(mConfig);
            mEngine.setContext(engineContext);
        } catch (NamingException nme) {
//            throw new JBIException (nme);
            String msg = I18n.loc("WLM-7011: Cannot get datasource");
            mLogger.log(Level.SEVERE, msg, nme);             
        }
    }
