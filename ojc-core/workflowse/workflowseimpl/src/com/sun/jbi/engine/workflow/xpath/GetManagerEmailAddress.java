...
/*
 * @(#)$Id: GetManagerEmailAddress.java,v 1.3 2010/02/15 19:24:47 fkieviet Exp $
 *
...
public class GetManagerEmailAddress implements Function {

    private static final Logger LOGGER = Logger.getLogger(GetManagerEmailAddress.class.getName());
    ...
    public Object invoke(ExpressionContext ctx, Object[] params) {
        ...
        if (params == null || params.length == 0) {
            ...
            if (userOrGroup == null) {
                throw new XpathException (I18n.loc(
                        "WLM-6084:  get-manager-email has no arguments and task is not claimed by anyone"));
            }
        }else {
        ...
        try {
            ...
            if (managerDN == null) {
                LOGGER.warning(I18n.loc(
                        "WLM-6082: Can not get manager email address for {0}, Ldap config : {1}", userOrGroup, ldapConfig.toString()
                        ));             
                return null;
            }
            
            //Get the email from DN
            email =  LDAPSearchUtil.getAttributeByDN(managerDN, ldapConfig.getEmailAttName(),  ldapConfig.getLDAPContext());
