...
public class OutboundAction implements Runnable {
    ...
    public void run() {
        if (mLog.isLoggable(Level.FINE)) {
            mLog.log(Level.FINE, "OutboundAction.Start_processing", mExchange.getExchangeId());
        }
        try {
            mProcessor.processMessage(mExchange);
        } catch (Exception ex) {
            mLog.log(Level.SEVERE, "OutboundAction.Unexpected_exception", ex);
        }
        if (mLog.isLoggable(Level.FINE)) {
            mLog.log(Level.FINE, "OutboundAction.Finished_processing", mExchange.getExchangeId());
        }
    }
