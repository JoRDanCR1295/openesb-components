...
public class DTELSEBootstrap implements Bootstrap {

    private static final Logger mLogger = Logger.getLogger(DTELSEBootstrap.class.getName());

    private InstallationContext mContext;
    private ObjectName mInstallerExtName;

    public DTELSEBootstrap() {
    }
    ...
    public void init(InstallationContext installContext)
        ...
        try {
            DTELSEConfigurationMBean installerExt = new DTELSEInstallerConfigurationMBean();
            ...
            if (!mbeanServer.isRegistered(mInstallerExtName)) {
                StandardMBean stdBean = new StandardMBean(installerExt, DTELSEConfigurationMBean.class);
                mbeanServer.registerMBean(stdBean,  mInstallerExtName);
            }
        } catch (Exception ex) {
            throw new JBIException("Caught exception while creating Installatoin Configuration MBean, failed to init dtelse component", ex);
        }
