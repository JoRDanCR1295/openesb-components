...
import com.sun.jbi.msmqbc.packaging.EndpointConfiguration;
import com.sun.jbi.msmqbc.packaging.EndpointConfigurationFactory;
import com.sun.jbi.msmqbc.packaging.InvalidConfigurationException;
import com.sun.jbi.msmqbc.packaging.WSDLConfigurations;
 ...
 * com.sun.jbi.msmqbc.packaging package should need to change.
 *
...
public class ServiceUnitImpl implements ServiceUnit {
    ...
    public ServiceUnitImpl(String suId, String suPath, ComponentContext context,
		RuntimeConfigurationMBean runtimeConfig,
            StatusProviderHelper statusProviderHelper, ChannelManager msmqChannelMgr,
    ...
    public void init(String serviceUnitRootPath) throws JBIException {
        ...
        mLogger.log(Level.INFO, "ServiceUnitImpl_INIT", new Object[] { getServiceUnitId() });
    ...
    public void start() throws JBIException {
        mLogger.log(Level.INFO, "ServiceUnitImpl_START_CALLED", new Object[] { getServiceUnitId() });
        ...
        try {
            while (it.hasNext()) {
                ...
                if (currEndpoint.getEndpointType() == EndpointType.OUTBOUND) {
                    ...
                    // Add MSMQ producer "channels"
                    addChannels(currEndpoint);
                } else {
                    // For inbound (we are the consumer), 
                    // the MSMQ "channel" has to be established first                    
                    // Add MSMQ consumer "channels"
                    addChannels(currEndpoint);
            ...
            if (currEndpoint != null) {
                mLogger.log(Level.SEVERE, "ServiceUnitImpl_ENDPOINT_ACTIVATION_ERROR", new Object[] {
                        currEndpoint.getServiceName().toString(), currEndpoint.getEndpointName(), ex });

                errMsg = mMessages.getString("ServiceUnitImpl_ENDPOINT_ACTIVATION_ERROR", new Object[] {
                        currEndpoint.getServiceName().toString(), currEndpoint.getEndpointName(), ex });
            }
            mLogger.log(Level.INFO, "ServiceUnitImpl_DEACTIVATE_ENDPOINTS_DUE_TO_ERROR");
            ...
            while (it2.hasNext()) {
                ...
                try {
                    ...
                    mLogger.log(Level.WARNING, "ServiceUnitImpl_MSMQ_CHANNELS_REMOVE_ERROR", new Object[] {
        ...
        mLogger.log(Level.INFO, "ServiceUnitImpl_START_SUCCEEDED", new Object[] { getServiceUnitId() });
    }
    ...
    public void stop() throws JBIException {
        mLogger.log(Level.INFO, "ServiceUnitImpl_STOP_CALLED", new Object[] { getServiceUnitId() });
        ...
        mLogger.log(Level.INFO, "ServiceUnitImpl_STOP_SUCCEEDED", new Object[] { getServiceUnitId() });
    }
    ...
    public void shutdown() throws JBIException {
        mLogger.log(Level.INFO, "ServiceUnitImpl_SHUTDOWN_CALLED", new Object[] { getServiceUnitId() });
        ...
        mLogger.log(Level.INFO, "ServiceUnitImpl_SHUTDOWN_SUCCEEDED", new Object[] { getServiceUnitId() });
    }
    ...
    private void activateEndpoint(Endpoint endpoint) throws JBIException {
        ...
        if (endpoint.getEndpointType() == EndpointType.OUTBOUND) {
            ...
            try {
                ...
                mLogger.log(Level.SEVERE, "ServiceUnitImpl_INBOUND_MSG_PROCESSORS_START_FAILED", new Object[] {
                        endpoint.getServiceName(), endpoint.getEndpointName(), ex });

                String errMsg = mMessages.getString("ServiceUnitImpl_INBOUND_MSG_PROCESSORS_START_FAILED",
        ...
        mLogger.log(Level.INFO, "ServiceUnitImpl_ENDPOINT_ACTIVATED", new Object[] { direction,
    ...
    private void deactivateEndpoint(Endpoint endpoint) throws JBIException {
        ...
        mLogger.log(Level.INFO, "ServiceUnitImpl_ENDPOINT_DEACTIVATED", new Object[] { direction,
                endpoint.getServiceName().toString(), endpoint.getEndpointName() });
    }

    private void addChannels(Endpoint endpoint) throws MSMQException {
        Map operations = endpoint.getMSMQOperations();
        Iterator iter = operations.keySet().iterator();
        List operationsList = new ArrayList();
        Channel msmqChannel = null;
        try {
            while (iter.hasNext()) {
                QName operation = (QName) iter.next();
                try {
                    msmqChannel = mMSMQChannelMgr.addChannel(endpoint, operation);
                } catch (ChannelManagerException ce) {
                ...
                try {
                    ...
                    if (endpoint.getEndpointType() == Endpoint.EndpointType.OUTBOUND) {
                        mLogger.log(Level.WARNING, "ServiceUnitImpl_OUTBOUND_CHANNEL_OPEN_FAILED",
            ...
            try {
                ...
                while (opsIter.hasNext()) {
                    ...
                    Channel channel = mMSMQChannelMgr.removeChannel(endpoint, op);
                    channel.close();
                }
            } catch (ChannelManagerException ce) {
                throw new MSMQException(ce); // propogate
    ...
    private void removeChannels(Endpoint endpoint) throws ChannelManagerException {
        Map operations = endpoint.getMSMQOperations();
        Iterator iter = operations.keySet().iterator();
        ...
        while (iter.hasNext()) {
            ...
            if (direction == EndpointType.OUTBOUND) {
                ...
                mMSMQChannelMgr.removeChannel(endpoint, operation);
            } else {
