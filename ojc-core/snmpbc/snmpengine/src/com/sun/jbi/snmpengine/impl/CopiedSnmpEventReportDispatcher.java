...
/*
 * @(#)CopiedSnmpEventReportDispatcher.java 
 *
...
public class CopiedSnmpEventReportDispatcher implements Runnable {
     ...
     */
    public CopiedSnmpEventReportDispatcher() 
     ...
     *
     */
    public CopiedSnmpEventReportDispatcher(int portNumber) 
	throws SocketException, IllegalArgumentException{
	this(new SnmpEngineParameters(),
	     new JdmkEngineFactory(),
	     portNumber,
     ...
     *
     */    
    public CopiedSnmpEventReportDispatcher(SnmpEngineParameters parameters, 
     ...
     *
     */    
    public CopiedSnmpEventReportDispatcher(SnmpEngine engine,
				     int portNumber)
	throws SocketException, IllegalArgumentException {
	...
	synchronized(trapListeners) {
		...
		final CopiedSnmpEventReportHandler handler =
		    new CopiedSnmpEventReportHandler(listener, trapPdu);
	...
	synchronized(informListeners) {
		...
		final CopiedSnmpEventReportHandler handler =
		    new CopiedSnmpEventReportHandler(listener, informPdu);
		handleCallback(handler);
	    }
	}
    }
    ...
    //BC:1
    protected 
    void sendPacket(final DatagramPacket respPacket) {
	if (respPacket == null) return;
	try {
    ...
    //BC:1 
    protected 
    void doHandlePacket(final int version,
				      final DatagramPacket packet) {
...
//BC:2
        // May need to resize to a larger size
        final byte dBuffer[] = new byte[SnmpPeer.defaultSnmpRequestPktSize];
...
//BC:1
                final DatagramPacket packet = new DatagramPacket(dBuffer, dBuffer.length);
...
//BC-1		handlePacket(new PacketHandler(version,packet));
...
//BC:*    
    protected void handlePacket(DatagramPacket packet) throws SnmpStatusException {
        final int version = SnmpMsg.getProtocolVersion(packet.getData());
...
//BC    
	...
	new ClassLogger(ClassLogger.LOGGER_SNMP,
			CopiedSnmpEventReportDispatcher.class.getName());
