...
/**
 * Bootstrap implementation for %PROJ_SHORT_NAME% %COMP_TYPE_DESC%.
 * 
 * @author %AUTHOR%
 */
public class %CLASS_PREFIX%Bootstrap extends AbstractBootstrap {
    public %CLASS_PREFIX%Bootstrap() {
    }
    ...
    protected Object initInstallMBean(ComponentContext ctx) throws JBIException {
        try {
            ...
            return new %CLASS_PREFIX%Config(ctx, getConfig());
        }
        catch (Exception e) {
            throw error(e, "%LOG_PREFIX%-6001: %CLASS_PREFIX%Bootstrap installation MBean failed to initialize: {0}",
                        e.getMessage());
        }
    }
}
