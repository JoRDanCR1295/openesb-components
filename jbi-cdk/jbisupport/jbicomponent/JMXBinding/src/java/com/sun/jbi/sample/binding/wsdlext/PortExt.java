...
/**
 * This interface and its implementation represents the java model for port extension element.
 * The inner classes PortExtImpl implements the interface and the PortExtSerializer provides
 * the serializer and deserializer implemenation. The implementation and serializer classes will
 * be registered with the ExtensionRegistry to process the port extension element in the wsdl
 * definition.
 *
    ...
    public static class PortExtSerializer extends AbstractExtensionSerializer implements ExtConstants {
        ...
        public void marshall(Class parentType, QName elementType, ExtensibilityElement extension,
            ...
            if ( serviceURL != null && serviceURL.trim().length() > 0 ) {
                buff.append(" " + ATTR_SERVICE_URL + "=\"" + serviceURL + "\"");
            }
            
            String mbean = extObj.getMBean();
            if ( mbean != null && mbean.trim().length() > 0 ) {
                buff.append(" " + ATTR_MBEAN + "=\"" + mbean + "\"");
            }
