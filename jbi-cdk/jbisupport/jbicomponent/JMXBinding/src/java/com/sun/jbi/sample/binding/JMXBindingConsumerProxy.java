...
/**
 * This class acts as a proxy to the external service consumer that can receive messages from
 * the external consumer using JMX interface and then initiate the InOut message exchange to
 * invoke a service provided by the component in the jbi environment.
 * 
 * this code shows how a synchronous send/receive messages from the delivery channel can be used
 * in InOut message exchange to complete a service invocation.
 *
 * @author chikkala
 */
