 ...
 * jbicomponent's build-impl.xml.
 * @author chikkala, j2seproject
 */
public final class JbiCompAntLogger extends AntLogger {
    
    /** Default constructor for lookup. */
    public JbiCompAntLogger() {}
    
    public boolean interestedInSession(AntSession session) {
        // Even if the initiating project is not a JbiCompProject, suppress these messages.
        // However disable our tricks when running at VERBOSE or higher.
        return session.getVerbosity() <= AntEvent.LOG_INFO;
    }
    
    private static boolean isJbiCompProject(File dir) {
        ...
        try {
            ...
            if (proj != null) {
                // Check if it is a JbiCompProject.
                return proj.getLookup().lookup(JbiCompProject.class) != null;
            }
        } catch (IOException e) {
    ...
    public boolean interestedInScript(File script, AntSession session) {
        if (script.getName().equals("build-impl.xml")) { // NOI18N
            ...
            if (parent != null && parent.getName().equals("nbproject")) { // NOI18N
                ...
                if (parent2 != null) {
                    return isJbiCompProject(parent2);
                }
            }
        }
        // Was not a JbiCompProject's nbproject/build-impl.xml; ignore it.
        return false;
    }
    ...
    public void messageLogged(AntEvent event) {
        // related to J2SEProject. May not need for JbiCompProject
