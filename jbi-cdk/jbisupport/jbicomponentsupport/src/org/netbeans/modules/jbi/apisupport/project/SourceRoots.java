...
public final class SourceRoots {
        ...
        ProjectManager.mutex().writeAccess(
                new Mutex.Action () {
                    public Object run() {
                        ...
                        NodeList nl = cfgEl.getElementsByTagNameNS(JbiCompProjectType.PROJECT_CONFIGURATION_NAMESPACE, elementName);
                        assert nl.getLength() == 1 : "Illegal project.xml"; //NOI18N
                        Element ownerElement = (Element) nl.item(0);
                        //Remove all old roots
                        NodeList rootsNodes = ownerElement.getElementsByTagNameNS(JbiCompProjectType.PROJECT_CONFIGURATION_NAMESPACE, "root");    //NOI18N
                        ...
                        for (Iterator it = newRoots.iterator(); it.hasNext();) {
                            ...
                            Element newRootNode = doc.createElementNS(JbiCompProjectType.PROJECT_CONFIGURATION_NAMESPACE, "root"); //NOI18N
    ...
    private void readProjectMetadata () {
        ...
        // It can be 0 in the case when the project is created by JbiCompProjectGenerator and not yet customized
        if (nl.getLength()==1) {
            NodeList roots = ((Element)nl.item(0)).getElementsByTagNameNS(JbiCompProjectType.PROJECT_CONFIGURATION_NAMESPACE, "root");    //NOI18N
