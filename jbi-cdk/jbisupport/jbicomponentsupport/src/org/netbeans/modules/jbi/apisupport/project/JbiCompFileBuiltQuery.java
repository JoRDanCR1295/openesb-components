...
public class JbiCompFileBuiltQuery implements FileBuiltQueryImplementation, PropertyChangeListener {
    ...
    JbiCompFileBuiltQuery (AntProjectHelper helper, PropertyEvaluator evaluator,
    ...
    private FileBuiltQueryImplementation createDelegate () {
        ...
        for (int i=0; i< srcRoots.length; i++) {
            ...
            to[i] = "${" + JbiCompProjectProperties.BUILD_CLASSES_DIR + "}/*.class"; // NOI18N
        }
        for (int i=0; i<tstRoots.length; i++) {
            from[srcRoots.length+i] = "${" + tstRoots[i] + "}/*.java"; // NOI18N
            to[srcRoots.length+i] = "${" + JbiCompProjectProperties.BUILD_TEST_CLASSES_DIR + "}/*.class"; // NOI18N
        }
