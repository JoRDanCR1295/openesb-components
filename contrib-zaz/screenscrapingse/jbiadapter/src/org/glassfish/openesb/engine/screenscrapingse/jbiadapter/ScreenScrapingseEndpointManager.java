...
/*
 * @(#)ScreenScrapingseEndpointManager.java
 *
...
import org.glassfish.openesb.engine.screenscrapingse.jbiadapter.ScreenScrapingseEndpoint.EntryType;
import org.glassfish.openesb.engine.screenscrapingse.process.ScriptInfoVO;
import org.glassfish.openesb.engine.screenscrapingse.scrconfig.FilterOneWay;
import org.glassfish.openesb.engine.screenscrapingse.scrconfig.Input;
import org.glassfish.openesb.engine.screenscrapingse.scrconfig.RequestReplyService;
import org.glassfish.openesb.engine.screenscrapingse.scrconfig.Scriptconfig;

public class ScreenScrapingseEndpointManager extends AbstractEndpointManager {
     ...
     */
    public ScreenScrapingseEndpointManager(ComponentContext ctx, StatusProviderHelper statusProviderHelper) {
        super(ctx);
    ...
    public Endpoint<ScriptInfoVO> createEndpoint(EndpointInfo info, ServiceUnit srvcUnit)
        ...
        if (!lJarFile.exists()) {
            String infomsg = I18n.loc("SSSE-6004 Script SU Additional Schemas jar is not there!!: {0}", lJarFile.getAbsolutePath());
            log().info(infomsg);
        }
        ...
        String portName = info.getEndpointName();
        String uniqueName =
                mStatusProviderHelper.createProvisioningEndpointIdentifier(
                serviceName,
                portName);
    ...
    private class ScriptConfigReader {
        ...
        public void parseScriptMap(File aScrFile) {
            String infomsg = I18n.loc("SSSE-6005 Inside the parseScriptMap Method.");

            log().info(infomsg);

            try {
                //Creating a JAXB Context.
                JAXBContext lJaxbCtx = JAXBContext.newInstance(org.glassfish.openesb.engine.screenscrapingse.scrconfig.ObjectFactory.class);
                ...
                String errormsg = I18n.loc("SSSE-6006: Failed to Parse the File {0} ", ex.getMessage());
