...
public class ScreenScrapingseExchangeHandler extends AbstractExchangeHandler {

    /** Constructs an {@link ExchangeHandler} for AleSE. */
    public ScreenScrapingseExchangeHandler(ComponentContext ctx) {
        super(ctx);
    }
    ...
    public void handleExchange(MessageExchange mex) throws JBIException {
        String i18n = I18n.loc("SSSE-5005: handle Exchnage {0}", mex.getExchangeId());
        ...
        try {
            ...
            switch (prs) {
                ...
                case IN_OUT_PROVIDER_ERROR: {
                    ...
                    logError(mex);
                     getContext().getCorrelationMap().remove(mex.getExchangeId());
                     break;
                }
                default: {
                    //status back in mex just log in
                    String err = I18n.loc("SSSE-6002: {0} does not support {1} exchange Pattern Role Status",
            ...
            i18n = I18n.loc("SSSE-6003: Failed to handle message exchange ({0}): {1} : {2}",
    ...
    protected void provisionService(MessageExchange mex)
         ...
         * 
         */
        ScreenScrapingseEndpoint endpt = (ScreenScrapingseEndpoint) findProvisioningEndpoint(mex);
        NormalizedMessage xIn = ((InOut) mex).getInMessage();
        if (xIn == null) {
            mex.setError(new JBIException("Cannot process NULL in message!"));
            mex.setStatus(ExchangeStatus.ERROR);
            //set the exchnage as error
            getContext().getMessagingChannel().send(mex);
            return;
        }
        // 2. Transform X ==================================================
        ...
        processedContent = ScreenScrapingseExecutorFactory.transform(content, endpt);
    ...
    protected boolean logStatus(MessageExchange msg) {
        if (ExchangeStatus.ERROR.equals(msg.getStatus())) {
            ...
            log().finer(I18n.loc(
                    "SSSE-6008: Exchange({0}) is complete with DONE status.",
                    msg.getExchangeId()));
        }
        return true;
    }
    ...
    protected void logError(MessageExchange msg) {
        ...
        log().warning(I18n.loc(
                "SSSE-6007: Exchange({0}) terminated with ERROR status by {1}.\n" +
    ...
    protected void continueProcessing(MessageExchange msg)
        ...
        String err = I18n.loc(
                "SSSE-6041 Not implemented yet {1}", msg.getExchangeId());
        log().severe(err);
